...
als Potenzreihe darzustellen, wir fragen also, ob es eine Folge $(a_n)_{n\in\mathbb{N}}$ gibt, so
dass 
\begin{equation}
  \label{eq:taylor}
 f(x) = \sum\limits_{n=0}^\infty a_n \cdot  x^n  
\end{equation}
gilt.  Falls eine solche Folge $(a_n)_{n\in\mathbb{N}}$ existiert, dann möchten wir diese Folge
...
\end{equation}
Um den Koeffizienten $a_1$ zu berechnen, differenzieren wir Gleichung (\ref{eq:taylor}) nach $x$:
...
\item[I.A.:] $k = 0$. \quad Es gilt \\[0.3cm]
              ...
              f^{(0)}(x) & = & f(x) \\[0.3cm]
                         ...
                         & = & \ds\sum\limits_{n=k}^\infty \bruch{n!}{(n-0)!} \cdot a_n \cdot x^{n-0}.
              \end{array}
              $ 
...
beliebig oft differenzierbar ist, die der Funktion $f$ zugeordnete \blue{Taylor-Reihe} als
\begin{equation}
...
$
\\[0.2cm]
definiert ist.  Diese Funktion ist in Abbildung \ref{fig:strange} gezeigt.  
Im Buch von Otto Forster \cite{forster:2011} wird gezeigt, dass für diese Funktion
die Werte sämtlicher Ableitungen an der Stelle $x = 0$ verschwinden.  Dies können Sie in der Zeichnung daran
erkennen, dass die Funktion in der Umgebung der Stelle $x = 0$ annähernd konstant ist.  Da die Ableitungen
im Punkt $x = 0$ alle den Wert $0$ haben, gilt  $\textsl{taylor}(f,x) = 0$.  
\eox

 \begin{figure}[!h]
   \centering
   \epsfig{file=Figures/strange.eps,scale=0.6}
   \caption{Die Funktion $\ds x \mapsto \exp\Bigl(-\frac{1}{x^2}\Bigr)$.}
   \label{fig:strange}
 \end{figure}
...
$n\in \mathbb{N}$ den \blue{Abbruch-Fehler vom Grad $n$} als
\\[0.3cm]
\hspace*{1.3cm}
$\textsl{error}_n(x) := f(x) - \displaystyle\sum\limits_{i=0}^n \bruch{f^{(i)}(0)}{i!} \cdot  x^i$.
\\[0.3cm]
Der Abbruch-Fehler gibt also an, wie groß der Fehler ist, wenn wir die Berechnung der Summe einer
...
Zunächst bemerken wir, dass die $k$-te Ableitung
des Abbruch-Fehlers vom Grad $n$ den Wert 0 hat
\\[0.3cm]
\hspace*{1.3cm}
$\textsl{error}_n^{(k)}(0) = 0$ \quad für alle $k=0,\cdots,n$. 
\\[0.3cm]
Dies folgt aus der Definition des Abbruch-Fehlers, denn wir hatten die Taylor-Reihe ja
gerade so definiert, dass die Ableitungen der Funktion $f$ an der Stelle $0$ mit den Ableitungen der Taylor-Reihe
übereinstimmen.   
...
\end{equation}
gilt. 
Für die Ableitung der Funktion $g_0(x) = x^{n+1}$ finden wir $\dfo g_0(x) = (n+1) \cdot  x^n$.
...
\end{equation}
gilt.
Für die Ableitung der Funktion $g_1(x) = (n+1)\cdot x^{n}$ finden wir $\dfo g_1(x) = (n+1) \cdot  n \cdot x^{n-1}$.
...
\blue{Taylor-Entwicklung} 
(\href{http://en.wikipedia.org/wiki/Brook_Taylor}{Brook Taylor}, 1685 -- 1731) der Funktion $f$ mit \blue{Lagrange'schem Restglied}
(\href{http://en.wikipedia.org/wiki/Lagrange}{Joseph Louis Lagrange}, 1736 -- 1813). 

\section{Beispiele von Taylor-Entwicklungen}
Wir zeigen nun, wie wir verschiedene transzendente Funktionen 
mit Hilfe der Taylor-Entwicklungen approximieren können.  Dadurch werden diese Funktionen dann
einer numerischen Berechnung zugänglich.  

\subsection{Berechnung des natürlichen Logarithmus}
...
$\ds\frac{f^{(0)}(0)}{0!} = f(0)$
\\[0.2cm]
undefiniert.  Alle weiteren Koeffizienten sind natürlich ebenfalls undefiniert.
Wir betrachten daher die Funktion $f(x)  := \ln(1 + x)$, denn diese Funktion ist für $x = 0$
definiert.  Zunächst berechnen wir die 
...
\item[I.A.:] $n=1$.  Es gilt
  ...
  \\[0.2cm]
  denn $n!$ ist für $n\in\mathbb{N}$ so definiert, dass $0! = 1$ gilt.
...
\\[0.3cm]
gilt.  Wegen $f(0) = \ln(1) = 0$ erhalten wir für die Taylor-Entwicklungen der Funktion $\ln(1+x)$ das Ergebnis 
\begin{equation}
...
Damit haben wir insgesamt $\textsl{taylor}\bigl(x \mapsto \ln(1+x), x \bigr) = \ln(1+x)$ für $x \in [0, 1]$ gezeigt und folglich können
wir
\\[0.2cm]
\hspace*{1.3cm}
\colorbox{red}{\framebox{\colorbox{orange}{
$\ds\ln(1+x) = \sum\limits_{k=1}^{\infty} (-1)^{k+1}\cdot \frac{x^k}{k}$ \quad für $x \in (-1, 1]$}}}
\\[0.2cm]
schreiben.\footnote{
  Die Formel gilt auch für für negative $x$, deren Betrag kleiner als 1 ist, aber das 
...
obigen Formel auf eine Genauigkeit von $10^{-9}$ berechnen zu können, müssten wir
...
\begin{equation}
  ...
  \begin{array}[b]{lcl}
   ...
   & = & \displaystyle\sum\limits_{k=1}^\infty (-1)^{k+1}\cdot  \bruch{x^k}{k} + \sum\limits_{k=1}^\infty \bruch{x^k}{k} \\[0.5cm]
  ...
  \end{array}
\end{equation}
denn $(-1)^{k+1} + 1$ hat für gerade Werte von $k$ den Wert $0$ und für ungerade Werte hat dieser
Term den Wert 2.
Setzen wir hier für $x$ den Wert $\ds\frac{1}{3}$ ein, so erhalten wir 
\\[0.2cm]
\hspace*{1.3cm}
...
dass wir bei der Abschätzung der Summe durch die geometrische Reihe den Faktor $\ds\frac{1}{2\cdot
  k+1}$ vernachlässigt haben.
\vspace*{0.3cm}

Das Verfahren, das wir oben benutzt haben um $\ln(2)$ zu berechnen, lässt sich
...
$
\\[0.2cm]
Bei gegebenem $r$ bestimmen wir also $x$ nach der Formel $x=\ds\frac{r-1}{r+1}$.  Für
...
\exercise
 Berechnen Sie die Taylor-Reihen für die Funktionen $x \mapsto \sin(x)$ und $x \mapsto \cos(x)$
 und geben Sie eine Abschätzung für den Abbruch-Fehler an.  Folgern Sie außerdem die
...
\\[0.2cm]
die auf 
\href{http://en.wikipedia.org/wiki/Leonard_Euler}{Leonard Euler} (1707 --- 1783) zurück geht.
\eox
...
Funktion $x \mapsto \arctan(x)$ in einer Taylor-Reihe entwickeln, so berechnen wir mit 
\href{http://www.wolframalpha.com}{WolframAlpha$^{\scriptsize\textregistered}$} die
...
\end{equation}
gefunden.  Für einen vollständigen Beweis dieser Formel müssten wir den Abbruch-Fehler
nach der Lagrange'schen Formel berechnen.  Das würde uns jetzt allerdings zu viel Zeit
kosten.

\subsection{Berechnung von $\pi$ über die Machin'sche Formel$^*$}
...
\\[0.3cm]
Überlegen wir, wie viele Glieder der Summe benötigt werden, um $\ds\frac{\pi}{4}$ auf eine Genauigkeit von $10^{-9}$ 
...
$
\\[0.2cm]
Wir  müssten wir also etwa 500 Millionen Terme aufsummieren um die geforderte Genauigkeit
zu erreichen.  

Es gibt noch eine elegantere Möglichkeit, die Kreiszahl $\pi$ mit Hilfe des Arkustangens zu
berechnen.  Um eine Formel zu erhalten, die schneller konvergiert, gehen wir von den
...
\\[0.2cm] 
Ersetzen wir hier noch die Brüche der Form $\ds\frac{\sin(x)}{\cos(x)}$ durch $\tan(x)$, so haben wir
...
\begin{equation}
  \label{eq:arctandouble}
2\cdot \arctan(x) = \arctan\left(\bruch{2\cdot x}{1 - x^2}\right)  
\end{equation}
Damit können wir die Machin'sche Formel (John Machin, 1686 -- 1751) zeigen:
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
äquivalent ist.  Wir wenden nun auf der linken Seite dieser Gleichung das Additions-Theorem des
Arkustangens an und benutzen für die rechte Seite die Formel (\ref{eq:arctandouble}).
Damit  finden wir
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Hier können wir auf der rechten Seite ein zweites Mal die Formel (\ref{eq:arctandouble})
anwenden und finden
\\[0.2cm]
\hspace*{1.3cm}
...
Bei der Machin'schen Formel müssen wir den Arcus-Tangens an den Stellen $\frac{1}{5}$ und $\frac{1}{239}$
berechnen.  Für diese Werte konvergiert die in Gleichung (\ref{eq:taylorArctan}) angegebene Reihe 
sehr schnell.


\exercises
\begin{enumerate}[(a)]
\item Berechnen Sie $\pi$ mit Hilfe der Machin'schen Formel auf eine Genauigkeit von $10^{-9}$.
\item Leiten Sie die folgende Formel aus dem Additions-Theorem des
...
mit einer Funktion $f$ an vorgegebenen Punkten übereinstimmen.  Sind $n+1$ Paare der Form
\\[0.2cm]
\hspace*{1.3cm}
$\pair(x_0,y_0)$,
$\pair(x_1,y_1)$,
...
\href{https://de.wikipedia.org/wiki/Interpolation_(Mathematik)}{Polynom-Interpolation} darin,
ein Polynom $p(x)$ vom Grad $n$ zu finden, so dass
\\[0.2cm]
\hspace*{1.3cm}
$\forall i \in \{0,1,\cdots,n\}: p(x_i) = y_i$
\\[0.2cm]
gilt.  Wir zeigen nun, dass diese Aufgabe lösbar ist:  Zu einer gegebenen Liste von 
$n+1$ verschiedenen \blue{Stützstellen} 
\\[0.2cm]
\hspace*{1.3cm}
$\bigl[x_0, x_1, \cdots, x_n]$
\\[0.2cm]
definieren wir das $k$-te \blue{Lagrange'sche Polynom} 
(\href{https://de.wikipedia.org/wiki/Joseph-Louis_Lagrange}{Joseph-Louis Lagrange}, 1736 -- 1813)
vom Grad $n$ für alle $k\in\{0,1,\cdots,n\}$ wie folgt: 
\begin{equation}
...
\end{enumerate}
Ist eine Liste $[x_0,x_1,\cdots,x_n]$ von $n+1$ verschiedenen Stützstellen gegeben, so
...
$k$ das \href{https://de.wikipedia.org/wiki/Kronecker-Delta}{Kronecker-Delta}
...
\end{equation}
Diese Eigenschaft werden wir durch einfaches Nachrechnen bestätigen.  Wir betrachten die Fälle
$j=k$ und $j\not=k$ getrennt:
\begin{enumerate}
...
konstruieren, die an den  Stützstellen $x_0,x_1,\cdots,x_n$ die vorgegebenen Werte 
$y_0$, $y_1$, $\cdots$, $y_n$ annehmen. Wir definieren 
\\[0.3cm]
\hspace*{1.3cm}
\colorbox{red}{\colorbox{orange}{\framebox{
$\displaystyle p(x) := \sum\limits_{k=0}^n y_k \cdot  L_k\bigl([x_0,\cdots,x_n];x\bigr)$ \quad mit \quad 
$\ds L_k\bigl([x_0,\cdots,x_n];x\bigr) := \prod\limits_{i=0 \atop i\not=k}^n \bruch{x-x_i}{x_k-x_i}$
.}}}
\\[0.3cm]
...
\blue{dividierten Differenzen} vom Rang $k$, die wir jetzt für alle $k= 1,\cdots,n$ durch
...
kann.  Dazu beweisen wir zunächst den folgenden Hilfs-Satz.  Dieser Satz ist eine Verallgemeinerung
des Satzes von Rolle (Satz \ref{satz:rolle}). 

\begin{Satz}
  Ist $f:\mathbb{R} \rightarrow \mathbb{R}$ eine Funktion, die $n$-mal differenzierbar
  ist und die außerdem $n+1$ verschiedene Null-Stellen 
  \\[0.2cm]
...
\end{enumerate}
Setzen wir in der gerade bewiesenen Behauptung für $k$ den Wert $n$ ein, so sehen wir, dass die Funktion $f^{(n)}(x)$ in dem Interval
...
$p(x)$ höchstens $n$ verschiedene Nullstellen hat.  Folgern Sie daraus, dass es zu $n+1$ gegebenen Paaren 
\\[0.2cm]
\hspace*{1.3cm}
$\pair(x_0,y_0)$, $\pair(x_1,y_1)$, $\cdots$, $\pair(x_n,y_n)$,
\\[0.2cm]
genau ein Polynom $p(x)$ vom Grad $\leq n$ gibt, so dass $p(x_i) = y_i$ für alle $i=0,1,\cdots,n$ gilt. \eox


\begin{Satz} 
  \label{satz:interpolationsFehler}
  Ist die Funktion $f:\mathbb{R} \rightarrow \mathbb{R}$ mindestens $(n+1)$-mal differenzierbar, ist $p(x)$ ein Polynom vom Grad
...
\\[0.2cm]
ist $(n+1)!$ mal der Koeffizient der Potenz $x^{n+1}$, denn alle Potenzen der Form $x^k$ mit $k \leq n$
verschwinden, wenn die $(n+1)$-te Ableitung gebildet wird.  Setzen
...
Einträge, die für die Erstellung der Tabelle notwendig ist. \eox
...
reellen Zahlen wird dann durch einen \blue{vollständigen}
\href{http://de.wikipedia.org/wiki/Metrischer_Raum}{\blue{metrischen Raum}} ersetzt.
...
\ref{chapter:folgen-und-reihen} über Folgen und Reihen hatten wir die Gleichung 
\\[0.2cm]
\hspace*{1.3cm}
$x = \cos(x)$
\\[0.2cm]
mit Hilfe eines Fixpunkt-Verfahrens gelöst.  Wir hatten damals induktiv eine Folge $\folge{x_n}$
...
$
\\[0.2cm]
auch eine Lösung der Fixpunkt-Gleichung $x = \cos(x)$ ist.  Wir wollen dieses Verfahren nun
verallgemeinern.  Wir nehmen dazu an, dass eine stetige Funktion $f$ gegeben ist und wir eine Lösung
...
ausgehend von einem weitgehend beliebig gewählten Startwert $x_1$ die übrigen Folgenglieder
...
dieser Grenzwert eine Lösung der Fixpunkt-Gleichung $x = f(x)$, denn es gilt
\\[0.2cm]
\hspace*{1.3cm}
...
von $f$ näher beieinander liegen als die Argumente, 
dann konvergiert die Folge.  Bevor wir das im Detail formal untersuchen, wollen wir die dahinter
liegende Anschauung verstehen.  Nehmen wir an, dass die Fixpunkt-Gleichung $f(x) = x$ eine Lösung
$\bar{x}$ hat und nehmen wir weiter an, dass eine Näherung $x_1$ für 
$\bar{x}$ gegeben ist.  Wenn nun die Funktionswerte von $f$ näher beieinander liegen als die
Argumente, dann können wir die Näherung $x_1$ zu einer Näherung $x_2$ verbessern, indem wir 
\\[0.2cm]
\hspace*{1.3cm}
$x_2 := f(x_1)$
\\[0.2cm]
...
$
\\[0.2cm]
wobei wir bei der letzten Ungleichung benutzt haben, dass die Funktionswerte von $f$ näher
beieinander liegen als die Argumente.  Um diese Argumentation wasserdicht zu machen, definieren wir
zunächst formal unter welchen Umständen die Funktionswerte einer Funktion näher beieinander liegen
als die Argumente.  Die nun folgende Definition ist etwas schärfer, als Sie es vielleicht im ersten
Moment vermuten würden.  Das werden wir später noch diskutieren.
\pagebreak

\begin{Definition}[kontrahierend] \lb
Eine Funktion $f:[a,b] \rightarrow [a,b]$ ist eine \blue{kontrahierende Abbildung} wenn es eine
reelle Zahl $q$ mit $q < 1$ gibt, so dass 
\\
\hspace*{2.3cm}
\colorbox{red}{\colorbox{orange}{\framebox{$\forall x,y \in [a,b]: |f(x) - f(y)| \leq q \cdot  |x - y|$}}}  
\\[0.2cm]
gilt.  Wir bezeichnen die Zahl $q$ als den \blue{Kontraktions-Koeffizienten}.
\eod
\end{Definition}
...
\\[0.2cm]
\hspace*{1.3cm}
 $\dfo \cos(x) = -\sin(x)$  \quad und für alle $x \in \bigl[0,\pi\bigr]$ gilt $\sin(x) \geq 0$.
\\[0.2cm]
...
\\[0.2cm]
denn wir hatten ja vorausgesetzt, dass die Ungleichung $|f'(t)| \leq q$ für alle
...
\proof
Wir starten den Beweis mit einer im ersten Moment skurril anmutenden Formel: 
\\[0.2cm]
\hspace*{0.8cm}
$\ds x_n - x_0 = (x_n - x_{n-1}) + (x_{n-1} - x_{n-2}) + \cdots + (x_2 - x_1) + (x_1 - x_0) = \sum\limits_{i=1}^n (x_i - x_{i-1})$.
\\[0.2cm]
Diese Summe wird als \blue{Teleskop-Summe} bezeichnet.
Daraus folgt sofort 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
wenn wir noch zeigen können, dass
...
Konkret zeigen wir durch vollständige Induktion, dass für alle $i\in\mathbb{N}$ die Ungleichung
\\[0.2cm]
\hspace*{1.3cm}
...
Abbildung auch stetig ist, können wir die Anwendung der Funktion $f$ mit der Grenzwert-Bildung
...
und $x_n$ können wir abschätzen, wenn wir $\bar{x}$ als unendliche Reihe schreiben und gleichzeitig
...
$|f(x) - f(y)| < |x - y|$
\\[0.2cm]
gilt.  Eine Funktion, die nur diese schwächere Bedingung erfüllt, wollen wir als eine 
\\[0.2cm]
\hspace*{1.3cm}
\blue{schwach kontrahierende Abbildung}
\\[0.2cm]
bezeichnen.  In der Tat kann gezeigt werden, dass auch für
eine schwach kontrahierende Abbildung $f$ die Folge $x_{n+1} := f(x_n)$ für beliebige Startwerte
gegen eine Lösung der Fixpunkt-Gleichung $x = f(x)$ konvergiert.  Allerdings ist die Konvergenz unter Umständen
...
$10^8$ Iterationen haben wir die Lösung der Gleichung $x = \sin(x)$ erst auf drei Stellen genau
berechnet.  Der Grund dafür ist, dass die Ableitung der Sinus-Funktion der Cosinus ist, und dieser hat
an der Stelle $x = 0$ den Betrag $1$.  Es ist zwar so, dass die Funktion $x \mapsto \sin(x)$
schwach kontrahierend ist, aber es gibt kein $q < 1$, so dass für alle $x,y \in \mathbb{R}$ die Ungleichung
\\[0.2cm]
\hspace*{1.3cm}
$| \sin(x) - \sin(y) | \leq q \cdot |x - y|$
\\[0.2cm]
erfüllt ist.  Das ist der Grund für die äußerst langsame Konvergenz der Folge $\folge{x_n}$.
\eox 
...
\\[0.2cm]
gefunden, welche dieselben Fixpunkte hat wie die ursprüngliche Funktion $f$.  Für die
Ableitung gilt 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
annimmt.  Da diese Gleichung aber komplexer als die ursprüngliche Gleichung
$x_{n+1} = \cos(x_n)$ ist, müssen wir damit rechnen, dass die Rundungsfehler höher sind als bei der
Iteration.  Zur Eliminierung dieser Rundungsfehler führen wir daher in Zeile 8
...
\end{equation}
gefunden.  Dieses Verfahren wird als  \blue{Newton'sches Verfahren} 
(\href{https://en.wikipedia.org/wiki/Isaac_Newton}{Sir Isaac Newton}, 1642 -- 1726)
bezeichnet.
Die Gleichung  (\ref{eq:NewtonZero}) lässt sich geometrisch interpretieren: Legen wir im Punkt
...
oszilliert die Folge $\folge{x_n}$ für große $n$ ebenfalls zwischen den Werten $0$ und $1$.
Wählen wir den Startwert nahe genug zur gesuchten Lösung, beispielsweise $x_0 = -1.5$, so
konvergiert das Verfahren gegen die Lösung 
$-1.769292354238631\cdots$.  Diese Beispiele zeigen, dass das Newton'sche Verfahren ohne weitere
Einschränkungen nicht zuverlässig ist.
...
konvergiert und wollen außerdem die Geschwindigkeit der Konvergenz des Verfahrens untersuchen.
Als erstes benötigen wir einen Hilfssatz über konvexe Funktionen.

\begin{Lemma}
  Die Funktion $f:[a,b] \rightarrow \mathbb{R}$ sei zweimal differenzierbar und konvex.
  Weiter sei die zweite Ableitung von $f$ stetig.
  Ist dann $x_0 \in [a,b]$ und definieren wir die lineare Funktion $g:[a,b] \rightarrow \mathbb{R}$ als
  \\[0.2cm]
  \hspace*{1.3cm}
...
Setzen wir hier für $x$ den Wert $x - x_0$ ein und berücksichtigen, dass
\\[0.2cm]
\hspace*{1.3cm}
 $f(x) = h(x - x_0)$, \quad $h(0) = f(x_0)$, \quad $h'(0) = f'(x_0)$ 
\quad und \quad $h^{(2)}(\chi) =  f^{(2)}(\chi + x_0)$
\\[0.2cm]
gilt, so folgt daraus für die Funktion $f$ die Gleichung
\\[0.2cm]
\hspace*{1.3cm}
...
$
\\[0.2cm] 
denn da wir angenommen hatten, dass die Funktion $f$ zweimal differenzierbar und konvex ist und dass weiterhin
$f^{(2)}(x)$ stetig ist, gilt
\\[0.2cm]
\hspace*{1.3cm}
$f^{(2)}(\varphi) \geq 0$ 
\\[0.2cm]
...
\begin{Satz}[Konvergenz des Newtonschen Verfahrens für monotone und konvexe Funktionen] \lb
...
$f(x_n) \geq 0$ ist, muss $x_n \geq \xi$ gelten.  Also ist die Folge $x_n$ durch $\xi$ nach unten beschränkt.
...
durch
eine Konstante $K$ abschätzen können, haben wir dann
\\[0.2cm]
\hspace*{1.3cm}
...
wie viele Iterationen höchstens notwendig sind um den Wert von $\sqrt[3]{2}$ auf eine Genauigkeit von
$10^{-101}$ zu berechnen.
\eox
...
\\[0.2cm]
zu lösen, haben Sie im ersten Semester kennengelernt: Es ist das Gauß'sche
Eliminations-Verfahren (\href{https://de.wikipedia.org/wiki/Carl_Friedrich_Gauss}{Carl Friedrich Gauß}, 1777 -- 1855).  
...
Matrizen heißen 
\href{https://de.wikipedia.org/w/index.php?title=Schwachbesetzte_Matrix&redirect=yes}{dünn besetzte Matrizen}.
Diese Art von Matrizen tritt beispielsweise bei   
...
sehr groß werden können.  Demgegenüber sind iterative Verfahren \blue{selbstkorrigierend}.
...
Das Verfahren, das wir auf diese Weise erhalten, wird als \blue{Gesamtschritt-Verfahren}
oder auch als \href{https://de.wikipedia.org/wiki/Jacobi-Verfahren}{Jacobi-Verfahren} 
...
\begin{Definition}[Starkes Zeilen-Summen-Kriterium]
  Eine $n\times n$ Matrix $A \in \mathbb{R}^{n\times n}$ erfüllt das \blue{starke Zeilen-Summen-Kriterium} 
...
Zahlen gewöhnt sind,  insbesondere gilt auch die \blue{Dreiecks-Ungleichung}, wir haben also
...
\\[0.2cm]
Das Verfahren, das auf dieser Formel basiert, wird als \blue{Einzelschritt-Verfahren} oder
auch 
\href{https://de.wikipedia.org/wiki/Einzelschrittverfahren}{Gauß-Seidel-Verfahren} 
...
Es lässt sich zeigen, dass das Gauß-Seidel-Verfahren zur Lösung immer dann konvergiert, wenn die Matrix
...
detaillierteren Diskussion aus Zeitgründen absehen müssen.
