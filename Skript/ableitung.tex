...
\\[0.3cm]
\hspace*{1.3cm}
$\ds \frac{\mathrm{d}\,f}{\mathrm{d}x}(\widehat{x}) = \lim\limits_{h \rightarrow 0}
...
als Funktion von $h$ auffassen.  Dieser Ausdruck wird auch als \emph{Differenzen-Quotient}
...
$\lim\limits_{h \rightarrow 0} \bruch{f(\widehat{x} + h) - f(\widehat{x})}{h}$
\\[0.3cm]
bezeichnen wir als \emph{Differential-Quotienten}.
\vspace*{0.3cm}

Die oben definierte Funktion $r(h)$ ist der Fehler, der entsteht, wenn wir die Funktion $f(x)$ durch ihre
...
\begin{array}[t]{lcl}
 ...
 & = &  \lim\limits_{h \rightarrow 0} \bruch{f(\widehat{x} + h) - f(\widehat{x})}{h} \cdot \lim\limits_{h \rightarrow 0} g(\widehat{x}+h) +
        f(\widehat{x}) \cdot \lim\limits_{h \rightarrow 0} \bruch{g(\widehat{x} + h) - g(\widehat{x})}{h} \\[0.4cm]
...
\end{enumerate}
Damit finden wir für die Differenzen-Quotienten der Funktion $g \circ f$ im Punkt
$\widehat{x}$: \\[0.3cm]
\hspace*{1.3cm}
$
\begin{array}[t]{lcl}
...
$
\\[0.3cm]
Die linke Seite der Gleichung stellt den Differenzen-Quotienten der Funktion $g\circ f$
...
stehenden Hilfsmitteln nicht möglich, da uns der Begriff der
\href{https://en.wikipedia.org/wiki/Uniform_convergence}{\emph{gleichmäßigen Konvergenz}}
einer Funktionen-Folge fehlt und wir nicht die Zeit haben, diesen Begriff einzuführen.  Um  Ihnen
zu zeigen, dass es an dieser Stelle durchaus Probleme geben könnte, präsentiere ich ein
...
\\[0.2cm]
Aus der Tatsache, dass für alle $x\in \mathbb{R}$ die Ungleichung $|\sin(x)| \leq 1$ gilt,  folgt
\\[0.2cm]
\hspace*{1.3cm}
$f(x) := \ds\lim\limits_{n\rightarrow\infty} f_n(x) = \lim\limits_{n\rightarrow\infty} \frac{\;\sin(n \cdot x)\;}{n} = 0$. 
\\[0.2cm]
...
$g(x) := f'(x)$.
\\[0.2cm]
Wenn wir den Limes $n \rightarrow \infty$ mit dem Prozess des Differenzieren vertauschen dürften, dann
...
\item Um den natürlichen Logarithmus ableiten zu können, betrachten wir die Gleichung 
      ...
      \remark
      An dieser Stelle haben wir nicht gezeigt, dass der natürliche Logarithmus differenzierbar
      ist.  Wir haben lediglich gezeigt, dass unter der Annahme, dass der natürliche Logarithmus
...
\item Um die Ableitung der Funktion $x \mapsto \sin(x)$ berechnen zu können, 
      ...
      \\[0.3cm]
       Aus dem Geometrie-Unterricht ist das Additionstheorem für den Sinus 
       bekannt: 
       \\[0.3cm]
      ...
      \hspace*{1.3cm}
      $\ds\df{}\sin(x) = \cos(x)$.
\item Die Ableitung des Kosinus könnte in analoger Weise berechnet werden, es ist aber 
...
\item Jetzt kann die Ableitung der Tangens-Funktion über die Quotienten-Regel berechnet 
      ...
      \end{array}
      $
\item Die Ableitung der Arcus-Tangens-Funktion kann nun mit demselben Trick berechnet werden,
...
Da $f$ im Punkt $\bar{x}$  ein lokales Maximum besitzt, finden wir ein $\varepsilon$, so dass 
\\[0.2cm]
\hspace*{1.3cm}
$\forall x \in \mathbb{R}:\bigl( |x - \bar{x}| < \varepsilon \rightarrow f(x) \leq f(\bar{x})\bigr)$
\\[0.2cm]
gilt.  Falls $n$ eine natürliche Zahl ist, für die $n>\frac{1}{\varepsilon}$ gilt, so liegt  $\bar{x} + \frac{1}{n}$ in der 
$\varepsilon$-Umgebung von $\bar{x}$.  Daher gilt für alle $n > \frac{1}{\varepsilon}$
\\[0.3cm]
\hspace*{1.3cm}
 $f\bigl(\bar{x} + \frac{1}{n}\bigr) \leq f(\bar{x})$. 
\\[0.3cm]
Damit gilt für den Differenzen-Quotienten
\\[0.3cm]
\hspace*{1.3cm}
...
$\varepsilon$-Umgebung von $\bar{x}$.  Daher gilt für alle natürlichen Zahlen $n$ mit $n > \frac{1}{\varepsilon}$
\\[0.3cm]
...
\remark
Der Satz bleibt richtig, wenn die Voraussetzung $f:\mathbb{R} \rightarrow \mathbb{R}$ zu 
$f:D \rightarrow \mathbb{R}$ abgeschwächt wird, wenn also $f$ nicht mehr auf ganz $\mathbb{R}$
definiert ist.  Dann muss allerdings zusätzlich sichergestellt werden,  
dass es eine $\varepsilon$-Umgebung von $\bar{x}$ gibt, die vollständig in dem Definitions-Bereich
$D$ liegt, es muss also ein $\varepsilon > 0$ geben, so dass $U_\varepsilon(\bar{x}) \subseteq D$ gilt.
\eox

\exercise
Überlegen Sie, warum diese zusätzliche Bedingung notwendig ist. \eox
...
Wir müssen darauf verzichten, den Beweis auszuführen, denn dazu müssten wir zunächst den Begriff einer
\blue{Teilfolge} einführen und dann den Satz von
\href{https://de.wikipedia.org/wiki/Satz_von_Bolzano-Weierstraß}{Bolzano-Weierstraß} beweisen.  Das würde 
uns zu lange aufhalten.
...
Verhältnis $\ds\frac{\;h\;}{r}$ der Höhe $h$ zum Radius $r$ für eine solche Dose.  \eox

\exercise
Berechnen Sie das Maximum und das Minimum der Funktion $x \mapsto x - |x^2 + 2 \cdot x - 3|$ in dem Intervall $[-4,2]$.  \eox

\begin{Satz}[\href{https://de.wikipedia.org/wiki/Mittelwertsatz_der_Differentialrechnung}{Mittelwert-Satz der Differential-Rechnung}, \href{http://de.wikipedia.org/wiki/Augustin-Louis_Cauchy}{Augustin-Louis Cauchy}, 1789--1857]
  \hspace*{\fill} \\
...
\begin{Satz}[Erweiterter Mittelwert-Satz]
  Sind die Funktionen $f,g:[a,b] \rightarrow \mathbb{R}$ für alle $x\in[a,b]$
  differenzierbar und gilt $g'(x) \not= 0$ für alle $x \in [a,b]$,
  so gilt: 
  \\[0.3cm]
  \hspace*{1.3cm}
...
Der folgende Satz ist für die praktische Berechnung von Grenzwerten unentbehrlich.  Wir haben
diesen Satz bereits mehrfach in der Vorlesung
``\href{https://github.com/karlstroetmann/Algorithms/blob/master/Lecture-Notes/algorithms.pdf}{Algorithmen und Komplexität}''
zur Berechnung von Grenzwerten benutzt.
...
auch iteriert anwenden lässt.  Beispielsweise gilt
\\[0.3cm]
\hspace*{1.3cm}
...
\begin{Definition}[monoton]
  Eine Funktion $f: D \rightarrow \mathbb{R}$ ist {\blue{monoton steigend}} g.d.w.
  \\[0.2cm]
  \hspace*{1.3cm}
  $\forall x,y \in D:\bigl(x < y \rightarrow f(x) \leq f(y)\bigr)$
  \\[0.2cm]
  gilt.  Die Funktion $f$ ist {\blue{streng monoton steigend}}, wenn die schärfere Bedingung
  \\[0.2cm]
  \hspace*{1.3cm}
  $\forall x,y \in D:\bigl(x < y \rightarrow f(x) < f(y)\bigr)$
  \\[0.2cm]
  erfüllt ist.  Weiter heißt  $f$ {\blue{monoton fallend}}, wenn
  \\[0.2cm]
  \hspace*{1.3cm}
  $\forall x,y \in D:\bigl(x < y \rightarrow f(x) \geq f(y)\bigr)$
  \\[0.2cm]
  gilt.  Analog ist $f$ {\blue{streng monoton fallend}}, falls die folgende Bedingung gilt:
  \\[0.2cm]
  \hspace*{1.3cm}
  $\forall x,y \in D:\bigl(x < y \rightarrow f(x) > f(y)\bigr)$.
  \eod
...
\item[``$\Rightarrow$'':]  Wir nehmen zunächst an, dass $f$ monoton steigend ist und zeigen, dass
      ...
      \\[0.3cm]
      Wir zeigen, dass der Differenzen-Quotient 
      \\[0.3cm]
      \hspace*{1.3cm} $\bruch{f(x+h) - f(x)}{h}$
      \\[0.3cm]
      für alle $h > 0$ größer oder gleich 0 ist. Aus $h > 0$ folgt $x + h > x$.  Aus der Monotonie von $f$
      folgt dann,  dass $f(x+h) \geq f(x)$ ist.  Also gilt $f(x+h)-f(x) \geq 0$ und daraus folgt
      \\[0.3cm]
      \hspace*{1.3cm} $\bruch{f(x+h) - f(x)}{h} \geq 0$.
      \\[0.3cm]
      Da die Ableitung $f'(x)$ als Grenzwert des Differenzen-Quotient für $h$ gegen $0$ definiert ist, muss
      $f'(x) \geq 0$  gelten.
\item[``$\Leftarrow$'':] Wir nehmen nun an, dass für alle $x\in D$ die Ungleichung $f'(x) \geq 0$ gilt und
      ...
      \\[0.3cm]
      gilt.  Aus $x < y$ folgt  $y - x > 0$ und aus $f(x) > f(y)$ folgt $f(y) - f(x) <0$.
...
\begin{Definition}[strenges lokales Minimum] \lb
  ...
  ein {\blue{strenges lokales Minimum}}, wenn gilt:
  \\[0.2cm]
  \hspace*{1.3cm}
...
\noindent
\textbf{Bemerkung}:  Der Begriff des {\blue{strengen lokalen Maximum}} lässt sich
analog definieren. 
\pagebreak
...
$x \mapsto f'(x)$ in der $\delta$-Umgebung von $x_0$ streng monoton steigend ist.  Da außerdem
...
streng monoton fallend und für $x > x_0$ streng monoton wachsend.  Dann muss $f$ aber ein strenges lokales
...
Eine Funktion $f:\mathbb{R} \rightarrow \mathbb{R}$ heißt {\blue{konvex}} genau dann, wenn 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Sie können dies sofort verifizieren, denn offenbar ist der Ausdruck $g(x)$  linear in der Variablen $x$ und
andererseits gilt
\\[0.2cm]
\hspace*{1.3cm}
...
Eine Funktion $f:\mathbb{R} \rightarrow \mathbb{R}$ heißt {\blue{konkav}} genau dann, wenn 
\\[0.2cm]
\hspace*{1.3cm}
$\forall x_1,x_2 \in \mathbb{R}:\forall t\in [0,1]: \Bigl(
  f\bigl(t \cdot x_1 + (1-t)\cdot x_2\bigr) \geq t \cdot f(x_1) + (1 - t) \cdot f(x_2) \Bigr)
$
\\[0.2cm]
...
\item[``$\Rightarrow$'':] Wir führen den Nachweis indirekt und nehmen an, dass es ein $x_0 \in \mathbb{R}$
  gibt, so dass $f''(x_0) < 0$ ist.  Ähnlich wie im Beweis von Satz \ref{satz:minimum} folgt daraus, dass
  ...
  \\[0.2cm]
  Damit hat die Funktion $g$ im Punkt $x_0$ ein strenges lokales Maximum.  Also gibt es eine $\delta_2$-Umgebung von
  $x_0$, so dass
  \\[0.2cm]
  \hspace*{1.3cm}
...
Wir definieren nun die \href{https://de.wikipedia.org/wiki/Eulersche_Zahl}{Eulersche Zahl}
$\mathrm{e}$ (\href{http://de.wikipedia.org/wiki/Leonhard_Euler}{Leonhard Euler},
...
\\[0.2cm]
Als Produkt differenzierbarer Funktionen ist die Funktion $g$ differenzierbar und es gilt
\\[0.2cm]
\hspace*{1.3cm}
$
\begin{array}[t]{lcll}
...
\\[0.2cm]
Multiplizieren wir diese Gleichung mit $\exp(\lambda \cdot x)$ und berücksichtigen, dass wir in der
...
$\mathrm{e}^x$ in den Fällen definieren können, in denen $x$ keine rationale Zahl ist. 
Es ist naheliegend, für beliebige reelle Zahlen $x \in \mathbb{R}$ den
Wert $\mathrm{e}^x$ als
\\[0.2cm]
\hspace*{1.3cm}
$\ds \mathrm{e}^x := \exp(x)$
