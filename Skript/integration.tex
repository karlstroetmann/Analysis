...
\blue{Treppen-Funktionen} zu approximieren.  Eine Treppen-Funktion ist eine Funktion, die
...
\\[0.2cm]
gefunden.  Die linke Summe bezeichnen wir als \blue{Unter-Summe}, die Summe auf der
rechten Seite der Ungleichungskette nennen wir \blue{Ober-Summe}.
...
\end{equation}
Diesen Grenzwert nennen wir auch das \blue{Integral} von $f$ in dem Intervall $[a,b]$.
...
\begin{enumerate}
\item \blue{Linearität:} Sind $f$ und $g$ zwei Funktionen, so dass
...
\hspace*{1.3cm} $\df F(x) = f(x)$
\\[0.2cm]
gilt.  In diesem Fall schreiben wir auch  $\ds F(x) = \int f(x) \, \dr x$.  Der Ausdruck
\\
\hspace*{1.3cm}
 $\ds\int f(x) \, \dr x$ 
\\[0.2cm]
wird als das \blue{unbestimmte Integral} der Funktion $f$ bezeichnet. \eod
\end{Definition}
...
\\[0.2cm]
und das heißt gerade, dass sich die beiden Stamm-Funktionen $F_1$ und $F_2$ nur um eine Konstante
unterscheiden.   
...
dieser Satz das zentrale Ergebnis dieser Vorlesung.  Historisch wurde dieser Satz unabhängig sowohl
...
dieses Satzes war für die Analysis von  fundamentaler Bedeutung, denn dieser Satz ermöglicht die
...
\end{equation}
Zusammen mit dem Satz über die Ableitung von Integralen zeigt dies, dass der Integral-Operator zum
Differential-Operator invers ist.   
...
durch $g$ so erhalten wir zusammen mit Gleichung (\ref{eq:subst1})  die \blue{Substitutions-Regel}:
\begin{equation}
  \label{eq:subst}
...
$\ds\int g(y)\, \dr y$
\\[0.2cm]
aus.  Hier führen wir die Variablen-Transformation $y = f(x)$ durch. Dann gilt 
\\[0.2cm]
\hspace*{1.3cm}
...
-\ln\bigl(|\cos(t)|\bigr) $.  
\\[0.2cm]
Das Rechnen mit den \blue{infinitesimalen} Größen $\dr x$ und $\dr t$ ist
offensichtlich intuitiver als die formale Anwendung der Substitutions-Regel.  \qed


\exercise
Bei den folgenden Aufgaben dürfen Sie die Substitutions-Regel in suggestiver Weise benutzen.
\begin{enumerate}[(a)]
...
\item Berechnen Sie das Integral
      ...
      \hint
      überlegen Sie, wie Sie sich durch Anwendung der Substitutions-Regel eine längliche Rechnung ersparen können.
      \eox
\end{enumerate}
...
\Leftrightarrow & \dfo \Bigl(u(x)\cdot v(x)\Bigr) - u'(x) \cdot  v(x) & = &  u(x) \cdot  v'(x) & \mbox{(Stamm-Funktion)} \\[0.3cm]
\Leftrightarrow & \bint\dfo \Bigl(u(x)\cdot v(x)\Bigr)\,\dr x - \int u'(x)\cdot  v(x)\,\dr x & = & \ds \int u(x) \cdot  v'(x) \,\dr x\\[0.3cm]
\end{array}
$
\\[0.2cm]
Die Stamm-Funktion von $\dfo\Bigl( u(x)\cdot v(x) \Bigr)$ ist natürlich $u(x)\cdot v(x)$, also haben wir
\begin{equation}
  \label{eq:partInt0}
...
$f\bigl(g(y)\bigr) = y$. Abbildung \ref{fig:umkehr.eps} zeigt eine monoton steigende Funktion $f$.  Durch
...
\\[0.2cm]
gegeben, welches wir nun berechnen.  Zunächst führen wir die Koordinaten-Transformation
...
in Gleichung (\ref{eq:partInt0})  $u(t) := \cos(t)$ und $v'(t) := \cos(t)$ .  Wegen $u'(t) = -\sin(t)$ und $v(t) = \sin(t)$ 
erhalten wir dann 
\\[0.2cm]
\hspace*{1.3cm}
$\ds I = \cos(t) \cdot \sin(t) \bigg|_0^{\frac{\pi}{2}} + \int_0^{\frac{\pi}{2}}\sin(t)\cdot\sin(t)\,\dr t = \int_0^{\frac{\pi}{2}}\sin^2(t)\,\dr t$.
\\[0.2cm]
...
$\ds h = \frac{b - a}{n}$.
\\[0.2cm]
Das Teilstück des Graphen, das sich von dem Punkt $\bigl\langle x, f(x) \bigr\rangle$  bis zu dem Punkt 
$\bigl\langle x + h, f(x+h) \bigr\rangle$ erstreckt, kann für kleine Werte von $h$ näherungsweise
...
$f(x) = \sqrt{1 - x^2}$
\\[0.2cm]
definiert ist.  Die Bogenlänge dieser Funktion ist ein Viertel des Umfangs eines Kreises mit Radius $1$.
Es gilt
\\[0.2cm]
\hspace*{1.3cm}
...
\hint
Diese Aufgabe ist zwar etwas anspruchsvoller, aber dafür ist die Lösung umso lehrreicher.  Es ist
sinnvoll, die Lösung in die folgenden Schritte aufzuteilen:
\begin{enumerate}[(a)]
\item Zunächst definieren wir die beiden Funktionen $\sinh(x)$ (lese: \blue{Sinus Hyperbolicus}) und
      $\cosh(x)$ (lese: \blue{Cosinus Hyperbolicus}) wie folgt:
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item Die Umkehrfunktion der Funktion $x \mapsto \sinh(x)$ können Sie auf den natürlichen
      ...
      Gleichung für $\alpha$ in Abhängigkeit von $y$.  Wenn Sie aus dieser Gleichung $\alpha$
      berechnet haben, dann finden Sie anschließend $x$ nach der Formel $x = \ln(\alpha)$.

      Die Umkehrfunktion, die wir auf diesem Weg berechnet haben, ist als 
      \href{https://de.wikipedia.org/wiki/Areasinus_Hyperbolicus_und_Areakosinus_Hyperbolicus}{\blue{Areasinus Hyperbolicus}}
      bekannt und wird von uns als $\mathtt{arsinh}(x)$ geschrieben.  In der angelsächsischen
      Literatur findet sich auch die Schreibweise $\sinh^{-1}(x)$ für diese Funktion.
...
\item Nun berechnen wir das Integral
      ...
      Stelle die Identität $\cosh^2(x) = 1 + \sinh^2(x)$ berücksichtigen. 
\item Das bei der Berechnung der Bogenlänge der Parabel auftretende Integral kann nun vermittels einer
      Substitution auf das Integral $I(x)$ zurück geführt werden. \eod 
\end{enumerate}
...
\\[0.3cm]
beschränkt.  Da diese Folge außerdem monoton ist, folgt die Konvergenz der Folge. \hspace*{\fill} $\Box$
\vspace*{0.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds I(\alpha) := \int\limits_{1}^\infty \frac{1}{t^\alpha}\, \dr t$ \quad für $\alpha > 0$.
\\[0.2cm]
Falls $\alpha \not= 1$ ist,  gilt 
\\[0.2cm]
\hspace*{1.3cm}
...
dann die \blue{Kepler'sche Fassregel} (Johannes Kepler, 1571 -- 1630).
\begin{equation}
  \label{eq:keplerFass}
...
also um das 16-fache! Wählen wir beispielsweise $n=20$ und berechnen das Integral
