...
Die  \href{https://de.wikipedia.org/wiki/Fourier-Analysis}{Fourier-Analyse} spielt in vielen Teilen der
Mathematik eine wichtige Rolle.  Eine Anwendung findet die Fourier-Analyse beispielsweise bei der Lösung von
Differential-Gleichungen.  Daneben ist die Fourier-Analyse ein wichtiges Hilfsmittel in der 
\href{https://de.wikipedia.org/wiki/Signalverarbeitung}{Signalverarbeitung}.
Wir beschränken uns in diesem Kapitel auf die Untersuchung von
\href{https://de.wikipedia.org/wiki/Fourierreihe}{Fourier-Reihen}, die nach
(\href{http://de.wikipedia.org/wiki/Joseph_Fourier}{Jean Baptiste Joseph Fourier}, 1768 - 1830) benannt sind. 
Dabei zerlegen wir eine \blue{periodische} Funktion in Sinus-Schwingungen verschiedener Frequenzen.  
Im Rahmen dieser Vorlesung werden wir die Fourier-Analyse nur zur Berechnung von unendlichen Reihen einsetzen,
denn für die anderen Anwendungen reicht die Zeit nicht aus.

Bei der Fourier-Analyse gehen wir davon aus, dass eine Funktion 
\mbox{$f:\mathbb{R} \rightarrow \mathbb{R}$} gegeben ist, die \blue{periodisch} mit der
Periode $2\cdot\pi$ ist, d.~h.~es gilt
\\[0.1cm]
\hspace*{1.3cm}
$\forall x \in\mathbb{R}: f(x+2\cdot\pi) = f(x)$.
\\[0.1cm]
Ein triviales Beispiel für eine periodische Funktion ist die konstante Funktion 
$x \mapsto c$.
...
Weitere Beispiele für periodische Funktionen sind die Funktionen 
\\[0.1cm]
\hspace*{1.3cm}
$x \mapsto \sin(n\cdot x)$ \quad und \quad $x \mapsto \cos(n\cdot x)$ 
\quad für $n\in\mathbb{N}$.
\\[0.1cm]
...
sind, so ist natürlich auch die Funktion
\\[0.1cm]
\hspace*{1.3cm}
$x \mapsto \alpha \cdot f(x) + \beta \cdot g(x)$ \quad für $\alpha,\beta\in\mathbb{R}$
\\[0.1cm] 
...
darstellen lässt.  Genauer definieren wir folgendes: 

\begin{Definition}[Fourier-Reihe]
  Es seien $\bigl(a_n)_{n\in\mathbb{N}}$ und $\folgea{b_n}$ Folgen reeller Zahlen.  Dann bezeichnen
  wir den Ausdruck
  \begin{equation}
    \label{eq:fourier}
  ...
  \end{equation}
  als die mit den Folgen  $\bigl(a_n)_{n\in\mathbb{N}}$ und $\folge{b_n}$ gebildete \blue{Fourier-Reihe.}
\end{Definition}

\section{Berechnung der Fourier-Koeffizienten}
Die zentrale Frage bei der \blue{Fourier-Analyse} ist es, für eine gegebene periodische Funktion
$f$ die \blue{Fourier-Koeffizienten} $a_k$ und $b_k$ zu berechnen.   Ist $f$ eine
...
\begin{equation}
  \label{eq:fourierEq}
    f(x) = \bruch{1}{2}\cdot a_0 \;+\; \sum\limits_{k=1}^\infty a_k\cdot \cos(k\!\cdot\!x) \;+\;
                                      \sum\limits_{k=1}^\infty b_k\cdot \sin(k\!\cdot\!x), 
\end{equation}
so können wir den Koeffizienten $a_0$ dadurch gewinnen, dass wir die Funktion $f(x)$ in
...
$
\\[0.3cm]
Das erste Integral auf der rechten Seite können wir ausführen, die anderen Integrale
vertauschen wir mit den unendlichen Reihen\footnote{
Eine genaue Analyse, wann diese Vertauschung zulässig ist, geht über den Rahmen dieser Vorlesung hinaus.}.
Das liefert
\begin{equation}
  \label{eq:fourierEq1}
\hspace*{-0.8cm}  
...
\begin{equation}
  \label{eq:fourierA0}
  \int_0^{2\cdot\pi}\!\!f(x)\,\dr x \; = \; \pi\cdot a_0 \quad \mbox{bzw.}\quad 
  a_0 = \bruch{1}{\pi} \cdot \int_0^{2\cdot\pi}\!\!f(x)\,\dr x
\end{equation}
Damit haben wir  den Koeffizienten $a_0$ bestimmt.  Um die Koeffizienten 
$a_k$ für $k\geq 0$ zu bestimmen, 
multiplizieren wir die Gleichung (\ref{eq:fourierEq}) mit $\cos(n\!\cdot\!x)$, wobei
$n\in\mathbb{N}$ ist.  Anschließend integrieren wir  über das
...
& = & \displaystyle \bruch{1}{2}\cdot a_0 \cdot \int_0^{2\cdot\pi} \cos(n\!\cdot\!x)\,\dr x  \\[0.5cm]
...
\end{array}  
\end{equation}
Wir berechnen als nächstes die Integrale, die in dieser Formel auftreten.
...
\\[0.3cm]
Wir berechnen $I_{n,k}$ durch partielle Integration, wobei wir 
\\[0.2cm]
\hspace*{1.3cm}
$u(x) := \cos(k\cdot x)$ \quad und \quad $v'(x) := \cos(n\cdot x)$ 
\\[0.2cm]
setzen. Für $n\not=0$  haben wir dann
\\[0.2cm]
\hspace*{1.3cm}
$u'(x) = -k \cdot \sin(k\cdot x)$ \quad und \quad  $\ds v(x) = \frac{1}{n} \cdot \sin(n\cdot x)$ 
\\[0.2cm]
und damit finden wir für die Integrale $I_{n,k}$ das Folgende:
\\[0.3cm]
\hspace*{1.3cm}
$
\begin{array}[t]{lcl}
...
& = & \displaystyle 
      \cos(k\!\cdot\!x)\cdot\bruch{1}{n}\cdot\sin(n\!\cdot\!x)\, \bigg|_0^{2\cdot\pi} + 
       \bruch{k}{n} \cdot \int_0^{2\cdot\pi} \sin(k\!\cdot\!x)\cdot\sin(n\!\cdot\!x)\, \dr x \\[0.5cm]
& = & \displaystyle 
      \bruch{k}{n} \cdot J_{n,k}.
\end{array}
$
\\[0.3cm]
Damit haben wir $I_{n,k}$ auf $J_{n,k}$ zurück geführt.  Jetzt berechnen wir $J_{n,k}$
durch partielle Integration.  Wir setzen
\\[0.2cm]
\hspace*{1.3cm}
$u(x) := \sin(k \cdot x)$ \quad und \quad $v'(x) := \sin(n \cdot x)$.
\\[0.2cm]
Für $n\not=0$ haben wir dann
\\[0.2cm]
\hspace*{1.3cm}
$u'(x) := k \cdot\cos(k \cdot x)$ \quad und \quad $\ds v(x) := -\frac{1}{n} \cdot \cos(n \cdot x)$.
\\[0.2cm]
Damit finden wir
\\[0.1cm]
\hspace*{1.3cm}
...
& = & \displaystyle 
   -\bruch{1}{n}\cdot\sin(k\!\cdot\!x)\cdot\cos(n\!\cdot\!x)\, \bigg|_0^{2\cdot\pi} + 
   \bruch{k}{n} \cdot \int_0^{2\cdot\pi} \cos(k\!\cdot\!x)\cdot\cos(n\!\cdot\!x)\, \dr x \\[0.5cm]
& = & \displaystyle
...
\\[0.3cm]
denn $\sin(k \cdot 2\cdot\pi) = \sin(0) = 0$.
Damit haben wir die Berechnung von $J_{n,k}$ auf $I_{n,k}$ zurück geführt.
...
\\[0.3cm]
\hspace*{1.3cm}
$I_{n,k} = 0$ \quad und \quad $J_{n,k} = 0$.
\\[0.3cm]
...
\\[0.1cm]
wobei $\delta_{n,k}$ das früher definierte Kronecker-Delta bezeichnet, für das die Gleichung
\\[0.2cm]
\hspace*{1.3cm}
$\delta_{n,k} = \left\{
\begin{array}{ll}
...
$
\\[0.2cm]
gilt.  Um die Gleichung \ref{eq:fourierAk1} nach den Koeffizienten $a_k$ auflösen zu können, müssen wir noch die 
Integrale 
\\[0.3cm]
\hspace*{1.3cm}
$H_{n,k} := \displaystyle\int_0^{2\cdot\pi} \cos(n\!\cdot\!x)\cdot\sin(k\!\cdot\!x)\, \dr x$
\\[0.3cm]
berechnen.  Wir könnten dieses Integral auf dieselbe Art berechnen, mit der wir oben die Integrale
...
Aus dem Additions-Theorem für die Sinus-Funktion 
\\[0.1cm]
\hspace*{1.3cm}
...
Für $n>0$ schreibt sich damit die Formel \ref{eq:fourierAk1} wie folgt
\\[0.3cm]
\hspace*{1.3cm}
$
\begin{array}[t]{lcl}
...
\begin{equation}
  \label{eq:fourierAk}
\colorbox{red}{\colorbox{orange}{\framebox{$a_n = \bruch{1}{\pi} \cdot\int_0^{2\cdot\pi}f(x)\cdot\cos(n\!\cdot\!x)\,\dr x$}}}   
\end{equation}
gefunden.  Vergleichen wir diese Formel mit der Formel \ref{eq:fourierA0}, so sehen wir, dass diese Gleichung
auch in dem Fall $n=0$ richtig ist.  Um die Koeffizienten $b_n$ zu berechnen, multiplizieren wir die Gleichung
\ref{eq:fourier} mit $\sin(n\!\cdot\!x)$ und integrieren über das Intervall $[0,2\!\cdot\!\pi]$. Dann  erhalten wir
nach einer Rechnung, die ganz analog zur Berechnung der Koeffizienten $a_k$ verläuft, das Ergebnis
\begin{equation}
  \label{eq:fourierBk}
 \colorbox{red}{\colorbox{orange}{\framebox{$b_n = \bruch{1}{\pi} \cdot \int_0^{2\cdot\pi}f(x)\cdot\sin(n\!\cdot\!x)\,\dr x$.}}}   
\end{equation}

\section{Konvergenz$^*$}
Wir müssen noch die Frage beantworten, für welche Funktionen $f$ die mit Hilfe der Gleichungen (\ref{eq:fourierAk})
(\ref{eq:fourierBk}) und (\ref{eq:fourier})  aufgestellte Fourier-Reihe gegen $f$ konvergiert. 
Wir wollen uns mit einem Satz begnügen, der im Wesentlichen auf 
\href{https://en.wikipedia.org/wiki/Peter_Gustav_Lejeune_Dirichlet}{Peter Gustav Lejeune Dirichlet} (1805 - 1859) zurück geht.
Zuvor benötigen wir noch zwei Definitionen.

\begin{Definition}[Einschränkung einer Funktion]
  Ist \mbox{$f:\mathbb{R} \rightarrow \mathbb{R}$} eine Funktion und ist $[a,b]$ ein nicht-leeres
  Intervall, so definieren wir die \blue{Einschränkung} von $f$ auf $[a,b]$ als die
  Funktion 
  \\[0.1cm]
  \hspace*{1.3cm}
  $f\!\upharpoonright_{[a,b]} : [a,b] \rightarrow \mathbb{R}$ \quad mit $f\!\upharpoonright_{[a,b]}(x) = f(x)$ für alle $x \in [a,b]$.
\end{Definition}

\begin{Definition}[stetig differenzierbar]
  Eine Funktion $f:[a,b] \rightarrow \mathbb{R}$ ist \blue{stetig differenzierbar}
  falls $f$ differenzierbar ist und außerdem die Ableitung $f':[a,b] \rightarrow
  \mathbb{R}$ stetig ist. 
\end{Definition}

\begin{Definition}[stückweise stetig differenzierbar]
  Eine Funktion $f:[0,2\!\cdot\!\pi] \rightarrow \mathbb{R}$ ist \blue{stückweise stetig differenzierbar} 
  falls es ein $n\in\mathbb{N}$ und Zahlen $x_0$, $x_1$, $\cdots$, $x_n$ gibt mit 
  \\[0.1cm]
  \hspace*{1.3cm}
  $0 = x_0\leq x_1 \leq x_2 \cdots \leq x_{n-1} \leq x_n = 2\!\cdot\!\pi$
  \\[0.1cm]
  gibt, so dass für alle $i=1,\cdots,n$ gilt: 
  \\[0.1cm]
  \hspace*{1.3cm}
  $f\!\upharpoonright_{[x_{i-1}, x_i]}$ ist stetig differenzierbar.  
\end{Definition}

Ein Beispiel für eine stückweise stetige Funktion sehen Sie in Abbildung
\ref{fig:saegezahn.eps} auf Seite \pageref{fig:saegezahn.eps}.  Die Funktion selber ist zwar stetig, aber in
den Punkten $0$, $\pi$ und $-\pi$ ist die Ableitung dieser Funktion nicht definiert.

\begin{Satz} Es gelte
  \begin{enumerate}
  \item $f:\mathbb{R} \rightarrow \mathbb{R}$ ist  stetig mit der Periode $2\cdot\pi$,
  \item $f\!\upharpoonright_{[0,2\!\cdot\!\pi]}$ ist  stückweise stetig differenzierbar,
  \item $a_k = \bruch{1}{\pi} \cdot\int\limits_0^{2\cdot\pi}f(x)\cdot\cos(k\!\cdot\!x)\,\dr x$ \quad für $k \in \mathbb{N}$,
  \item $b_k = \bruch{1}{\pi}\cdot\int\limits_0^{2\cdot\pi}f(x)\cdot\sin(k\!\cdot\!x)\,\dr x$ \quad für $k \in \mathbb{N}\symbol{92}\{0\}$.  
  \end{enumerate}
  Dann gilt \\[0.1cm]
  \hspace*{1.3cm}
  $f(x) = \bruch{1}{2}\cdot a_0 \;+\; \sum\limits_{k=1}^\infty a_k\cdot \cos(k\!\cdot\!x) \;+\;
...
\noindent
Der Beweis dieses Satzes benötigt Hilfsmittel, die im Rahmen der Vorlesung aus Zeitgründen nicht eingeführt werden können.

\section{Beispiele}
Um es gleich bei der Berechnung der Fourier-Koeffizienten einfacher zu haben, definieren
wir für eine Funktion $f:\mathbb{R} \rightarrow \mathbb{R}$ die Begriffe \href{https://de.wikipedia.org/wiki/Gerade_und_ungerade_Funktionen}{gerade und ungerade}: 
\begin{enumerate}
\item $f$ ist \blue{gerade}   g.d.w. $\forall x\in\mathbb{R}: f(-x) =  f(x)$.

      Geometrisch heißt dies, dass die Funktion $f$  invariant gegenüber einer Spiegelung an der $y$-Achse ist.
\item $f$ ist \blue{ungerade} g.d.w. $\forall x\in\mathbb{R}: f(-x) = -f(x)$.

      Die geometrische Interpretation ist, dass die Funktion $f$ invariant gegenüber einer Punkt-Spiegelung an dem
      Punkt $\pair(0,0)$ ist.  
\end{enumerate}
Ist die Funktion $f:\mathbb{R} \rightarrow \mathbb{R}$ ungerade und ist $f$ integrierbar,
so gilt für beliebige Zahlen $a$ 
\begin{equation}
...
\\[0.3cm]
In dem Integral über das Intervall $[-a,0]$ führen wir die Variablen-Transformation $y =
-x$ durch.  
Dann gilt $\dr y = - \dr x$, $y(-a) = a$ und $y(0) = 0$.  Damit haben wir
\\[0.3cm]
\hspace*{1.3cm}
$
...
\\[0.3cm]
Ist die Funktion $f:\mathbb{R} \rightarrow \mathbb{R}$ gerade, so kann ein Integral über
...
\subsection{Fourier-Analyse der Sägezahn-Funktion}
Wir berechnen nun die Fourier-Reihe für die \blue{Sägezahn-Funktion} $s:\mathbb{R} \rightarrow \mathbb{R}$,  
...
\item Die Koeffizienten $a_n$ ergeben sich als 
      ...
      $
      \\[0.3cm]
      denn die Funktion $x \mapsto s(x) \cdot \cos(n\!\cdot\!x)$ ist einerseits
      periodisch mit der Periode $2\cdot\pi$ und anderer\-seits eine gerade Funktion.
      ...
      \\[0.1cm]
      Falls $n > 0$ ist, berechnen wir das Integral mit Hilfe einer partieller Integration.  Wir setzen
      \\[0.2cm]
      \hspace*{1.3cm}
      $u(x) = x$ \quad und \quad $v'(x) = \cos(n \cdot x)$.      
      \\[0.2cm]
      Dann gilt
      \\[0.2cm]
      \hspace*{1.3cm}
      $u'(x) = 1$ \quad und \quad $\ds v(x) = \frac{1}{n}\cdot \sin(n \cdot x)$. 
      \\[0.2cm]
      ...
      $
      \begin{array}[t]{lcl}
         a_n &= &\displaystyle \bruch{2}{\pi} \cdot\left( x \cdot \bruch{1}{n}\cdot\sin(n\cdot x) \bigg|_0^{\pi} - 
      ...
      \\[0.3cm]
      \hspace*{1.3cm}
      $a_0 = \pi$, \quad $a_{2\cdot n+1} = \bruch{-4}{\pi\cdot n^2}$ \quad und \quad $a_{2\cdot(n+1)} = 0$ \quad für $n\in \mathbb{N}$.
...
\\[0.3cm]
gefunden.  Setzen wir hier für $x$ den Wert $\pi$ ein, so ergibt sich wegen
\\[0.2cm]
\hspace*{1.3cm}
 $\cos\bigl((2\cdot k+1)\cdot \pi\bigr) = \cos(\pi) = - 1$
\\[0.2cm]
...
\\[0.2cm]
zu berechnen.  Zunächst zerlegen wir die Reihe in einen Teil, der nur über die geraden Indices 
läuft und einen Teil, der über die ungeraden Indices läuft: 
\\[0.3cm]
\hspace*{1.3cm}
$
\begin{array}[t]{clcl}
...
\\[0.3cm]
gezeigt.  Die Frage nach dem Wert dieser Reihe war 1644 als \href{https://de.wikipedia.org/wiki/Basler_Problem}{Baseler Problem} von
\href{https://de.wikipedia.org/wiki/Pietro_Mengoli}{Pietro Mengoli} gestellt worden.  Führende Mathematiker des
17-ten Jahrhunderts hatten sich erfolglos mit dieser Frage beschäftigt.
Im Jahre 1735 gelang es \href{https://en.wikipedia.org/wiki/Leonhard_Euler}{Leonard Euler} (1707 - 1783), dieses Problem zu lösen.
\pagebreak
...
\item Berechnen Sie mit Hilfe der Fourier-Reihe von $p$ einen Wert für die Reihe
      \\[0.3cm]
      \hspace*{1.3cm}
