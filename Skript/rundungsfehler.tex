...
\item Ein \blue{Approximations-Fehler} tritt auf, wenn wir einen Wert $\lambda$ berechnen
      wollen, zu dessen Berechnung nur eine Näherungsformel existiert.  Oft ist der
      ...
      berechnen können.  Wollen wir beispielsweise die Euler'sche Zahl $\mathrm{e}$ nach der Formel
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      also als Approximation für $\mathrm{e}$ eine Reihe der Form
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item Zusätzlich zum Approximations-Fehler gibt es noch die \blue{Rundungsfehler}, die im Laufe der Rechnung 
      ...
      bis zu 46 Bits notwendig sein, um alle Stellen hinter dem Komma korrekt wiedergeben zu können.
...
Die Antwort ist, dass für $n = 1673864$ der Wert $1/n$ so klein ist, dass die Summe
\\[0.2cm]
\hspace*{1.3cm}
$\ds 16.0 + \frac{1}{n}$
\\[0.2cm]
...
definierten 32-Bit-Architektur erhalten wir für $\mathtt{eps}$ den Wert
\\[0.2cm]
\hspace*{1.3cm}
...
einer  Grundrechenoperation durch die Maschinen-Konstante $\mathtt{eps}$ beschränkt ist.
