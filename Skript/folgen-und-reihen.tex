...
\begin{Definition}[Folge]
  Eine Funktion $f\!:\! \mathbb{N}\symbol{92}\{0\} \rightarrow \mathbb{R}$ bezeichnen wir als eine  \blue{reellwertige Folge}. 
  Eine Funktion $f\!:\! \mathbb{N}\symbol{92}\{0\} \rightarrow \mathbb{C}$ bezeichnen wir als eine  \blue{komplexwertige Folge}. \eod
\end{Definition}
...
\hspace*{\fill} \\
Eine Folge $\folge{a_n}$ \blue{konvergiert} gegen den \blue{Grenzwert} $g$, falls gilt:
\\[0.2cm]
\hspace*{1.3cm}
...
Eine Folge $\folge{a_n}$ ist \blue{beschränkt}, falls es eine \blue{Schranke} $S$ gibt, so dass 
\\
\hspace*{1.3cm}
...
\examples
\begin{enumerate}
\item Die Folge $\folge{(-1)^n}$ ist durch die Schranke $S=1$ beschränkt, denn offenbar gilt
      \\[0.2cm]
      \hspace*{1.3cm}
      $ \bigl|(-1)^n\bigr| = 1 \leq 1$.
\item  Die Folge $\folge{n}$ ist nicht beschränkt, denn sonst gäbe es eine Zahl $S$, so dass
...
Eine Folge $\folge{a_n}$ ist \blue{monoton steigend} falls 
\\[0.2cm]
\hspace*{1.3cm}
...
\eod
\end{Definition}
Ein Beispiel für eine monoton fallende Folge ist die Folge $\ds\Folge{\frac{1}{n}}$, denn es
gilt
\\[0.2cm]
\hspace*{1.3cm}
$
...
\end{Satz}
\textbf{Beweis}:  Wir definieren zunächst die Menge $M$ als die Menge aller unteren Schranken der Folge $\folge{a_n}$
\\[0.2cm]
\hspace*{1.3cm}      
...
Eine Folge $\folge{a_n}$ heißt \blue{Cauchy-Folge} 
...
\proof
Wenn $\rule[-8pt]{0pt}{0pt}\folge{a_n}$ eine Cauchy-Folge ist, dann finden wir eine Zahl
...
\item Anschließend zeigen wir, dass die Folge $\folge{a_n}$ gegen $s$ konvergiert.
\end{enumerate}
...
\item[(b)] Es seien $a,b \in \mathbb{R}$ und zusätzlich gelte $a > 0$ und $b > 0$.  
           ...
           \item Ist die Funktion $f:\mathbb{R} \rightarrow \mathbb{R}$ \blue{stetig} und ist die Folge
                 $\folge{a_n}$ konvergent, so ist auch die Folge $\folge{f(a_n)}$ konvergent und es gilt
                 \\[0.2cm]
                 \hspace*{1.3cm}
...
Ist $\folge{a_n}$ eine Folge, so definieren wir die Folge der \blue{Partial-Summen} $\folge{s_n}$
durch die Festsetzung
\\[0.2cm]
\hspace*{1.3cm}
$\ds s_n := \sum\limits_{i=1}^n a_i. $
\\[0.2cm]
Diese Folge bezeichnen wir auch als unendliche  \blue{Reihe}.  Die Folge $\folge{a_n}$
...
Die Folge $\folge{a_n}$ ist eine \blue{Null-Folge} wenn gilt:
\\[0.2cm]
\hspace*{1.3cm}
...
\exercise
Die Folge $\folge{a_n}$ sei eine monoton fallende \blue{Null-Folge}.  Zeigen Sie, dass dann
\\[0.2cm]
\hspace*{1.3cm}
...
Dazu definieren wir zunächst die der Folge $\folge{a_n}$ zugeordnete 
