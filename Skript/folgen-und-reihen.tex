...
$\bigl(f(n)\bigr)_{n\in\mathbb{N}}$ oder $\bigl(f_n\bigr)_{n\in\mathbb{N}}$ oder noch k\"urzer als $\bigl(f_n\bigr)_n$.  Dabei ist $f_n$ als Abk\"urzung f\"ur $f(n)$ zu verstehen. 
...
ist die Menge $M$ sicher nicht leer.  Au{\ss}erdem ist die Menge $M$ nach oben beschr\"ankt, eine
obere Schranke ist das Folgenglied  $a_1$.  Da die Struktur $\langle \mathbb{R}, 0, 1, +, \cdot, \leq\rangle$ ein vollst\"andig geordneter
...
gilt.
\begin{enumerate}
\item[I.A.:] $n=1$: Einerseits haben wir f\"ur $n=1$
             \\[0.2cm]
             \hspace*{1.3cm}      
...
\\[0.2cm]
hei{\ss}t \href{https://en.wikipedia.org/wiki/Geometric_series}{\emph{geometrische Reihe}}.  Es gilt
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Da $q \not =1$ voraus gesetzt ist, k\"onnen wir diese Gleichung durch $(1-q)$ 
dividieren.  Dann erhalten wir f\"ur die Partial-Summen $s_n$
den Ausdruck:
\\[0.2cm]
\hspace*{1.3cm}      
...
\remark 
Die \href{http://de.wikipedia.org/wiki/Geometrische_Reihe}{\emph{geometrische Reihe}} ist 
...
\\[0.2cm]
Um diese Ungleichung nachzuweisen, zeigen wir zunächst durch vollst\"andige Induktion, dass f\"ur alle
...
\\[0.2cm]
\hspace*{1.3cm}
\colorbox{red}{\framebox{\colorbox{orange}{$\ds e^z = \sum\limits_{n=0}^{\infty} \frac{z^n}{n!}$}}} \quad f\"ur alle $z \in \mathbb{C}$.
\eox
...
\\[0.2cm]
als \colorbox{orange}{\emph{formale Potenz-Reihe}}.  Wichtig ist hier, dass $x$ keine feste Zahl ist, sondern
...
\\[0.2cm]
eine formale Potenz-Reihe, f\"ur alle $n \in  \mathbb{N}$ gelte $a_n \not= 0$ und die Folge
\\[0.2cm]
\hspace*{1.3cm}
...
$\ds\sum\limits_{n=0}^\infty a_n\cdot z^n$ 
\\[0.2cm]
eine formale Potenz-Reihe.  F\"ur alle $n \in  \mathbb{N}$ sei $a_n \not= 0$ und es gelte
\\[0.2cm]
\hspace*{1.3cm}      
$\ds R = \lim\limits_{n\rightarrow\infty} \left|\frac{a_n}{\;a_{n+1}\;}\right|$.  
\\[0.2cm]
...
\proof
Wir f\"uhren den Beweis indirekt und nehmen an, dass die obige Reihe konvergiert.  
...
\\[0.2cm]
Wegen $R < |u|$ gilt $\varepsilon > 0$.  Also finden wir ein $K$ so dass f\"ur alle nat\"urlichen
Zahlen $n$, die gr\"o{\ss}er-gleich $K$ sind, die Ungleichung
\\[0.2cm]
\hspace*{1.3cm}
$\ds\left|\Bigl|\frac{a_n}{\;a_{n+1}\;}\Bigr| - R\right| < \frac{1}{2} \cdot (|u| - R)$
\\[0.2cm]
gilt.  Wir zeigen, dass dann f\"ur alle 
$n \geq K$ die Ungleichung
\\[0.2cm]
\hspace*{1.3cm}
$\ds \left|\frac{a_{n}}{a_{n+1}}\right| < \frac{1}{2} \cdot (R + |u|)$ 
\\[0.2cm]
gilt.  Zun\"achst haben wir:
\\[0.2cm]
\hspace*{1.3cm}
$
\begin{array}{cl}
...
\\[0.2cm]
Sch\"atzen wir in der Ungleichung $(2)$ den Wert $|a_{n+1}| \cdot |u|$ durch die Ungleichung $(1)$
ab, so erhalten wir die Ungleichung
\\[0.2cm]
\hspace*{1.3cm}
$\ds |a_{n+2}| \cdot |u|^2 > |a_{n}| \cdot \left(\frac{|u|}{\frac{1}{2} \cdot (R+|u|)}\right)^2$. \hspace*{\fill} $(3)$
\\[0.2cm]
Diese Ungleichung k\"onnen wir zu der Ungleichung
\\[0.2cm]
\hspace*{1.3cm}
$\ds |a_{n+h}| \cdot |u|^h > |a_{n}| \cdot \left(\frac{|u|}{\frac{1}{2} \cdot (R+|u|)}\right)^h$. \hspace*{\fill} $(5)$
\\[0.2cm]
verallgemeinern.  Formal k\"onnten wir die Ungleichung $(5)$ durch eine triviale Induktion nach $h$
beweisen.  W\"ahlen wir nun ein festes $n_0$, dass gr\"o{\ss}er als $K$ ist, so gilt f\"ur beliebige
...
\\[0.2cm]
Damit wird die rechte Seite der Ungleichung $(6)$ beliebig gro{\ss} und folglich ist
\\[0.2cm]
\hspace*{1.3cm}
...
\item Zeigen Sie durch vollst\"andige Induktion, dass f\"ur alle nat\"urlichen Zahlen $n$ die Formel
      ...
      \href{http://de.wikipedia.org/wiki/Binomischer_Lehrsatz}{\emph{binomische Lehrsatz}}
      bezeichnet.
...
\href{http://de.wikipedia.org/wiki/Erzeugende_Funktion}{\emph{erzeugende Funktion}} $f(x)$ als die
...
\\[0.2cm]
vereinfachen.  Wegen $a_0 = 0$ f\"allt der erste Term weg.  Lassen wir die Summe bei $n=0$ starten,
so erhalten wir 
\\[0.2cm]
\hspace*{1.3cm}
$(x + x^2) \cdot f(x) = \sum\limits_{n=0}^\infty a_{n} \cdot x^n - a_0 \cdot x^0 - a_1 \cdot x^1$.
\\[0.2cm]
Ber\"ucksichtigen wir abermals, dass $a_0 = 0$ ist, so k\"onnen wir dies zu
\\[0.2cm]
\hspace*{1.3cm}
...
$x = (1 - x - x^2) \cdot f(x)$ 
\\[0.2cm]
umstellen k\"onnen.  Teilen wir hier noch durch $(1-x-x^2)$, so haben wir f\"ur die erzeugende Funktion
$f(x)$ die Gleichung
\\[0.2cm]
\hspace*{1.3cm}
...
der rechten Seite zu finden.  Dazu bestimmen wir zun\"achst die Nullstellen des Polynoms $x^2 + x - 1$:
\\[0.2cm]
\hspace*{1.3cm}
$
\begin{array}[t]{clcl}
...
Wir klammern in der Gleichung f\"ur $x^2 + x - 1$ den Term $x_1 \cdot x_2$ aus und erhalten
\\[0.2cm]
\hspace*{1.3cm}
...
$.
\\[0.2cm]
Damit k\"onnen wir die erzeugende Funktion $f(x)$ in der Form
\\[0.2cm]
\hspace*{1.3cm}
...
$
\\[0.2cm]
Um $\alpha$ und $\beta$ bestimmen zu k\"onnen, multiplizieren wir mit dem Hauptnenner und erhalten die Gleichung 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Da diese Gleichung f\"ur alle $x$ gelten muss, erhalten wir durch einen Koeffizientenvergleich die
beiden Gleichungen 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
gilt.  Insgesamt haben wir damit die folgende Gleichung f\"ur die erzeugende Funktion $f(x)$ gefunden:
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
und setzen $q = x \cdot \varphi$ bzw.~$q = x \cdot \overline{\varphi}$, so k\"onnen wir 
die Br\"uche auf der rechten Seite der Gleichung $(*)$ durch geometrische
...
\\[0.2cm]
Setzen wir nun f\"ur $f(x)$ die Reihe $\ds\sum\limits_{n=0}^\infty a_n \cdot x^n$ ein und fassen die die
