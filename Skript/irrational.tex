\chapter[Die Zahlen $\pi$ und $e$ sind irrational]{Die Kreiszahl $\pi$ und die Euler'sche Zahl $\mathrm{e}$ sind irrational}
In diesem Kapitel zeigen wir, dass sowohl die Kreiszahl $\pi$, die als Fläche eines Kreises mit dem Radius 1
definiert ist, als auch die Euler'sche Zahl $\mathrm{e}$, die durch
\\[0.2cm]
\hspace*{1.3cm}
$\ds\mathrm{e} := \exp(1) := \sum\limits_{k=0}^\infty \bruch{1}{k!}$
\\[0.2cm] 
definiert ist, irrational sind.  Da der Nachweis der Irrationalität von $\mathrm{e}$ einfacher ist, beginnen wir
damit.

\section{Die Euler'sche Zahl $\mathrm{e}$ ist irrational}
Nach Definition von $\mathrm{e}$ gilt
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds r_n := \mathrm{e} - s_n = \sum\limits_{k=0}^\infty \bruch{1}{k!} - \sum\limits_{k=0}^n \bruch{1}{k!} 
...
$
\begin{array}[t]{lcll}
r_n & = & \ds\sum\limits_{k=n+1}^\infty \bruch{1}{k!}                                      \\[0.2cm]
...
\begin{Theorem}
  Die Eulersche Zahl $\mathrm{e}$ ist irrational.
\end{Theorem}

\proof
Wir führen den Beweis indirekt und nehmen an, dass $\mathrm{e}$ rational ist.  Dann gibt es natürliche Zahlen $p,q \in \mathbb{N}$ mit
$q \geq 1$ und
\\[0.2cm]
\hspace*{1.3cm}
$\mathrm{e} = \bruch{p}{q}$.
\\[0.2cm]
Wir betrachten den Ausdruck $q! \cdot r_q = q! \cdot (\mathrm{e} - s_q)$ und setzten dort für $\mathrm{e}$ den Wert $\ds\frac{p}{q}$ ein:
\\[0.2cm]
\hspace*{1.3cm}
...
\item[I.A.:] $n=0$.
     ...
     $\sin(\pi) = \sin(0) = 0$
     \\[0.2cm]
     finden wir damit für das Integral $I$ den Wert
     \\[0.2cm]
     \hspace*{1.3cm}
...
denn sonst könnten wir zu beliebigem $r \in \mathbb{R}$ einfach das Polynom
\\[0.2cm]
\hspace*{1.3cm}
$p_r(x) := x - r$
\\[0.2cm]
...
Es lässt sich zeigen, dass die Menge aller Polynome mit ganzzahligen Koeffizienten abzählbar ist.  Damit
...
\begin{Theorem}[Charles Hermite, 1873] \lb
  Die Eulersche Zahl $\mathrm{e}$ ist transzendent.
\end{Theorem}
...
\\[0.2cm]
finden Sie eine Ausarbeitung des Nachweises der Transzendenz von $\mathrm{e}$, einen Nachweis der Transzendenz
