...
möglichen Längen anzugeben.  Es gibt mehrere Wege, die Menge $\mathbb{R}$ der reellen Zahlen so zu
konstruieren, so dass die Gleichung 
\\[0.2cm]
\hspace*{1.3cm}
$r^2 = 2$
\\[0.2cm]
in $\mathbb{R}$ eine Lösung hat.  Bevor wir mit dieser Konstruktion beginnen, wollen wir die Menge
...
sich alle weiteren Eigenschaften der reellen Zahlen ableiten lassen.  Es lässt sich sogar zeigen,
dass diese Axiomatisierung in dem folgenden Sinne vollständig ist:
Ist $\mathbb{K}$ ein vollständig angeordneter Körper, so ist $\mathbb{K}$ \emph{isomorph} zu
$\mathbb{R}$: Im Klartext heisst dies, dass es eine Abbildung
\\[0.2cm]
\hspace*{1.3cm}
$\varphi:\mathbb{K} \rightarrow \mathbb{R}$
\\[0.2cm]
gibt, die jedem Element $\alpha \in \mathbb{K}$ genau ein Element $\varphi(\alpha) \in \mathbb{R}$
zuordnet und die mit der Addition und der Multiplikation verträglich ist, für alle $x,y \in
\mathbb{K}$ gilt also 
\\[0.2cm]
\hspace*{1.3cm}
...
\item Was genau sind reelle Zahlen?
      ...
      Diese Frage hat zwar zunächst einen philosophischen Charakter, aber es zeigt sich, dass wir diese
      Frage zuerst beantworten müssen, bevor wir die erste Frage in Angriff nehmen können.
\end{enumerate}
...
technisch nicht ganz einfach ist, werden wir diesen Abschnitt im Rahmen der Vorlesung nicht
besprechen.   Die in diesem Abschnitt präsentierten Details sind zwar eine gute Übung zur
Mengenlehre,  werden aber für den weiteren Verlauf der Vorlesung nicht benötigt.  Der Abschnitt ist
daher zum Selbststudium für die Studenten gedacht, die an den Grundlagen der Mathematik interessiert sind.
...
Eine Struktur $\mathcal{K} = \langle K, 0, 1, +, \cdot \rangle$ ist ein \emph{Körper}, falls gilt:
\begin{enumerate}
...
\item $\langle K, 0, + \rangle$ ist eine kommutative Gruppe.
      ...
      \begin{enumerate}
        ...
        \item $x + y = y + x$,
        \item $0 + x = x$,
        \item $\exists y \in K: x + y = 0$.

              Dasjenige $y \in K$, für welches $x + y = 0$ gilt, ist dann eindeutig
              bestimmt und wird mit $-x$ bezeichnet.
        \end{enumerate}
\item $\langle K \backslash \{ 0 \}, 1, \cdot \rangle$ ist ebenfalls eine kommutative Gruppe,
      ...
      \item $1 \cdot x = x$,
      \item $x \not= 0 \Rightarrow \exists y \in K: x \cdot y = 1$.
      \end{enumerate}
...
\begin{Definition}[Lineare Ordnung]
  ...
  \begin{enumerate}
  \item $M$ ist eine Menge,
  \item $\leq$ ist eine binäre Relation auf $M$, es gilt also
        \\[0.2cm]
        \hspace*{1.3cm}
...
\begin{Definition}[Geordneter Körper]  \hspace*{\fill} \linebreak
  ...
  \emph{geordneter Körper}, falls 
  \begin{enumerate}
  \item $\langle K, 0, 1, +, \cdot, \rangle$ ein Körper und
  \item $\langle K, \leq \rangle$
  \end{enumerate}
  eine lineare Ordnung ist.  \edx
\end{Definition}

\remark
Die Struktur $\langle \mathbb{Q}, 0, 1, +, \cdot, \leq \rangle$ ist ein geordneter Körper, wenn wir
die Operationen $+$, $\cdot$ und $\leq$ wie im ersten Semester vorgeführt auf den rationalen Zahlen
definieren.  Diese Struktur ist allerdings für die Zwecke der Analysis noch nicht ausreichend,
da sie in einer noch näher zu spezifizierenden Weise unvollständig ist.  Zur Präzisierung dieser Aussage
benötigen wir den Begriff einer \emph{vollständigen Ordnung}, die ihrerseits
auf dem Begriff des \emph{Supremums} basiert, den wir jetzt einführen.

\begin{Definition}[Supremum]
Es sei $\langle M, \leq \rangle$ eine lineare Ordnung.
Eine Menge $A \subseteq M$ ist \emph{nach oben beschränkt}, falls es ein
...
obere Schranke von $A$ ist, wenn also 
\\[0.2cm]
\hspace*{1.3cm}
...
\noindent
\textbf{Bemerkung}:
Das Paar $\langle \mathbb{Q}, \leq \rangle$ ist keine vollständige Ordnung, denn wenn wir die Menge
$A$ als
\\[0.2cm]
\hspace*{1.3cm}
...
daran, dass die kleinste obere Schranke von $A$ die Zahl $\sqrt{2}$ ist, aber aus dem ersten
...
Eine Menge $B \subseteq M$ ist \emph{nach unten beschränkt}, falls es ein
 $y \in M$ gibt, so dass 
\\[0.2cm]
\hspace*{1.3cm}
...
Ein Element $i \in M$ ist das \emph{Infimum} einer Menge $B$, wenn $i$ die größte
...
\begin{Definition}[Vollständig angeordneter Körper]
  Eine Struktur  $\mathcal{K} = \langle K, 0, 1, +, \cdot, \leq \rangle$  ist ein 
  \emph{vollständig angeordneter Körper} genau dann, wenn die Struktur $\mathcal{K}$
  ein geordneter Körper ist und zusätzlich das Paar $\langle K, \leq \rangle$ eine vollständige
  Ordnung ist. \edx
\end{Definition}
...
Einen Beweis diese Behauptung können wir jetzt noch nicht erbringen, denn wir haben bisher nicht
definiert, wie die Menge $\mathbb{R}$ der reellen Zahlen konstruiert wird.
Diese Konstruktion werden wir im nächsten Abschnitt mit Hilfe der sogenannten
\emph{Dedekind-Schnitte} liefern.  Für praktische Rechnungen ist diese 
viel zu schwerfällig.  Wir geben daher zum Abschluss dieses
Abschnitts noch eine Konstruktion der reellen Zahlen als unendliche Dezimalbrüche an.  Dazu
...
\item Die Zahlen $b_k$ werden für alle $k \in \mathbb{N}$ induktiv definiert.
      \begin{enumerate}
      \item[I.A.:] $k=1$.  Wir setzen
                   \\[0.2cm]
                   \hspace*{1.3cm}
                   $\ds b_1 := \max\Bigl(\Bigl\{ z \in \{ 0, 1, \cdots, 9 \} \mid m + z \cdot \frac{1}{10} \leq x\Bigr\}\Bigr)$.
      \item[I.A.:] $k \mapsto k+1$.  Nach Induktions-Voraussetzung sind die Zahlen $b_1$, $\cdots$, $b_k$
...
$\ds x = m + \sum\limits_{k=1}^{\infty} b_k \cdot \frac{1}{10^{k}}$.
\\[0.2cm]
Diese Behauptung können wir allerdings noch nicht beweisen, denn wir haben ja noch gar nicht formal
...
obere Schranke $y$ für diese Menge gibt, die echt kleiner als $x$ ist.  Für den Rest der Vorlesung
...
Eigenschaften in Form von Axiomen angegeben.  Was noch fehlt ist nun die Konstruktion der reellen
...
rationalen Zahlen  zwischen $M_1$ und $M_2$ eine Lücke.  Die Idee ist nun, die reellen Zahlen gerade
als diese Lücken zu definieren um dadurch sicherzustellen, dass es bei den
reellen Zahlen selber keine Lücken mehr gibt.  
...
angeordneten Körper konstruieren.
...
\\[0.2cm]
Wir werden später die Menge $\mathbb{R}$ der reellen Zahlen als diese Menge
$\mathcal{D}$ definieren.  Die nächste Aufgabe zeigt, wie wir auf
der Menge $\mathcal{D}$ eine lineare Ordnung erzielen können. 
...
Konstruktion der reellen Zahlen im Detail beschrieben.   
Auch das Buch ``\emph{Principles of  Mathematical Analysis}'' von Walter Rudin \cite{rudin:1976}
diskutiert die Konstruktion der reellen Zahlen mit Hilfe von Dedekind-Mengen ausführlich. 
