...
so dass $r^2 = 2$ ist, denn $\sqrt{2}$ ist irrational.  Folglich reichen die rationalen Zahlen für die Geometrie
...
Ist $\mathbb{K}$ ein vollständig angeordneter Körper, so ist $\mathbb{K}$ \colorbox{orange}{\emph{isomorph}} zu
...
Eine Struktur $\mathcal{K} = \langle K, 0, 1, +, \cdot \rangle$ ist ein \colorbox{gold}{\emph{Körper}}, falls gilt:
\begin{enumerate}
...
\begin{Definition}[Geordneter Körper]  \hspace*{\fill} \linebreak
  ...
  \colorbox{gold}{\emph{geordneter Körper}}, falls 
  \begin{enumerate}
...
\colorbox{gold}{\emph{nach oben beschränkt}}, falls es ein
...
\begin{Definition}[Vollständige Ordnung]
  ...
  ist eine \colorbox{gold}{\emph{vollständige}} Ordnung genau denn, wenn folgendes gilt:
  \begin{enumerate}
...
\colorbox{gold}{\emph{nach unten beschränkt}}, falls es ein
 $y \in M$ gibt, so dass 
\\[0.2cm]
\hspace*{1.3cm}
...
Ein Element $i \in M$ ist das \colorbox{gold}{\emph{Infimum}} einer Menge $B$, wenn $i$ die größte
...
\begin{Definition}[Vollständig angeordneter Körper] \hspace*{\fill} \linebreak
  Eine Struktur  $\mathcal{K} = \langle K, 0, 1, +, \cdot, \leq \rangle$  ist ein 
  \colorbox{gold}{\emph{vollständig angeordneter Körper}} genau dann, wenn die Struktur $\mathcal{K}$
...
Einen Beweis diese Behauptung können wir jetzt noch nicht erbringen, denn wir haben bisher noch gar nicht
...
\colorbox{gold}{\emph{Dedekind-Schnitte}} liefern.  Für praktische Rechnungen ist diese 
...
\colorbox{gold}{\emph{unendliche Dezimalbrüche}}, die
