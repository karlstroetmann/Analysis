...
\item Wir zeigen, dass die Menge $I$ kein Maximum hat.
      ...
      \end{enumerate}
\end{enumerate}
Um den Beweis abzuschließen zeigen wir nun, dass $I$ tatsächlich die kleinste untere Schranke der Menge
...
\item Wir definieren $S := \bigcup \mathcal{M}$.  Es lässt sich zeigen, dass die so definierte Menge
      ...
      Weiter können Sie dann nachweisen, dass tatsächlich $S = \sup(M)$ gilt.  
\item Alternativ können Sie auch versuchen auszunutzen, dass wir bereits gezeigt haben, dass für
      eine nach unten beschränkte Menge $\mathcal{M} \subseteq \mathcal{D}$
...
\item Wir zeigen, dass $A \cdot B$ nach unten abgeschlossen ist.
      ...
      \item Fall: $z > 0$.  Dann definieren wir
            ...
            Damit haben wir insgesamt $u \cdot y \in A$.  Es gilt aber
            \\[0.2cm]
            \hspace*{1.3cm}
...
\end{enumerate}
\renewcommand{\labelenumi}{\arabic{enumi}.}
Bisher haben wir das Produkt $A \cdot B$ 
nur für den Fall definiert, dass $A$ und $B$ beide positiv sind.  Es gibt
aber noch drei weitere Fälle:
\begin{enumerate}
\item[2.] Fall: $A$ ist positiv, aber $B$ ist nicht positiv.  Dann ist $-\!B$ positiv und wir können
          \\[0.2cm]
          \hspace*{1.3cm}
          $A \cdot B := -\!\bigl(A \cdot (-\!B)\bigr)$
          \\[0.2cm]
          definieren.
\item[3.] Fall: $B$ ist positiv, aber $A$ ist nicht positiv.  Dann setzen wir
          \\[0.2cm]
          \hspace*{1.3cm}
          $A \cdot B := -\!\bigl((-\!A) \cdot B\bigr)$.
\item[4.] Fall: Weder $A$ noch $B$ ist positiv.  Wir definieren
          \\[0.2cm]
          \hspace*{1.3cm}
          $A \cdot B := (-\!A) \cdot (-\!B)$.
\end{enumerate}
Nun müssten wir noch nachweisen, dass für die so definierte Multiplikation zusammen mit der oben definierten
Addition die Körper-Axiome gelten.  Der detailierte Nachweis dieser Axiome ist für eine einführende
Vorlesung aber zu zeitaufwendig. 


\noindent
\textbf{Literatur-Hinweise}
In dem Buch ``\emph{Grundlagen der Analysis}'' von Edmund Landau \cite{landau:1930} wird diese
Konstruktion der reellen Zahlen im Detail beschrieben.   Auch das Buch 
``\emph{Principles of Mathematical Analysis}'' von Walter Rudin
\cite{rudin:1976} zeigt die Konstruktion der reellen Zahlen mit Hilfe von Dedekind-Mengen im Detail.
...
die Menge der reellen Zahlen keine Löcher hat.  Diese Erkenntnis und die Konstruktion der reellen
...
\cite{dedekind:1872}, das im Jahre 1872 erschienen ist, beschrieben hat.
