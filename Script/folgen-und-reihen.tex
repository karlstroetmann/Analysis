...
\end{Satz}
\textbf{Beweis}:  Wir definieren zunächst die Menge $M_1$ als als die Menge aller unteren Schranken der Folge $\folge{a_n}$
\\[0.2cm]
\hspace*{1.3cm}      
...
Es gibt nun zwei Möglichkeiten:  Die Menge $M_1$ hat ein Maximum oder die Menge $M_1$ hat kein
Maximum.  Wir betrachten diese beiden Fälle getrennt.
\begin{enumerate}
\item Das Maximum der Menge $M_1$ existiert.  In diesem Fall definieren wir
      \\[0.2cm]
      \hspace*{1.3cm}
      $s := \max(M_1)$.
      \\[0.2cm]
      ...
      \\[0.2cm]
      gilt.
\item Die Menge $M_1$ hat kein Maximum.  In diesem Fall definieren wir die Menge $M_2$ als das
      Komplement der Menge $M_1$:
      \\[0.2cm]
      \hspace*{1.3cm}
$ \begin{array}{lcl}
...
Um zu zeigen, dass das Paar $\pair(M_1,M_2)$ ein Dedekind-Schnitt ist, müssen wir
 noch 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Folglich ist $\pair(M_1,M_2)$ ein Dedekind-Schnitt und wegen der Vollständigkeit der reellen
...
\noindent
\textbf{Lösung}:
Wir konstruieren das Infimum als Trennungs-Zahl eines Dedekind-Schnitts.
Wir definieren den Dedekind-Schnitt $\pair(M_1,M_2)$ wie folgt:
\begin{enumerate}
\item $M_1 := \bigl\{ u \in \mathbb{R} \mid \forall x \in M : u \leq x \bigr\}$.
      ...
      gilt.
\item Damit $\pair(M_1,M_2)$ ein Dedekind-Schnitt wird, muss $M_2$ das Komplement
...
\end{enumerate}
Um nachzuweisen, dass $\pair(M_1,M_2)$ ein Dedekind-Schnitt ist, müssen wir nur noch
...
\\[0.2cm]
Also ist $\pair(M_1,M_2)$ ein Dedekind-Schnitt und nach dem Vollständigkeits-Axioms 
...
\item $s$ ist eine untere Schranke von $M$.
      ...
      \\[0.2cm]
      Da $s$ die Trennungs-Zahl des Dedekind-Schnitts $\pair(M_1,M_2)$ ist,
...
\item $s$ ist die größte untere Schranke von $M$.
      ...
      Für die Trennungs-Zahl $s$ des Dedekind-Schnitts $\pair(M_1,M_2)$ gilt:
      \\[0.2cm]
...
\item Da das Paar $\pair(M_1,M_2)$ eine Dedekind-Schnitt werden soll, definieren wir
...
Dedekind-Schnitt.  Aus der Vollständigkeit der reellen Zahlen folgt nun, dass eine
