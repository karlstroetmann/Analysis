...
\\[0.2cm]
\hspace*{1.3cm}
\colorbox{orange}{$\ds e^z = \sum\limits_{n=0}^{\infty} \frac{z^n}{n!}$} \quad für alle $z \in \mathbb{C}$.
\eox
...
\hspace*{1.3cm}
 $\ds\Reihe{a_i}$ 
\\[0.2cm]
heißt \emph{absolut konvergent} genau dann, wenn die Reihe der Absolutbeträge
\\[0.2cm]
\hspace*{1.3cm}
 $\ds\Reihe{|a_i|}$ 
\\[0.2cm]
...
$
\\[0.2cm]
Jetzt können wir zeigen, dass die Reihe $\sum\limits_{n=0}^\infty a_n\cdot u^n$ das
...
\\[0.2cm]
\hspace*{1.3cm}
$a_0 := 0$, \quad $a_1 := 1$ \quad und \quad $a_{n+2} := a_{n+1} + a_n$ für alle $n \in \mathbb{N}_0$
\\[0.2cm]
...
$x = (1 - x - x^2) \cdot f(x)$ 
\\[0.2cm]
umschreiben.  Teilen wir hier noch durch $(1-x-x^2)$, so haben wir für die erzeugende Funktion
$f(x)$ die Gleichung
\\[0.2cm]
\hspace*{1.3cm}
...
der rechten Seite zu finden.  Indem Sie die Nullstellen des Polynoms $1 - x - x^2$ bestimmen, können
...
\\[0.2cm]
darstellen.  Ersetzen wir die Brüche auf der rechten Seite dieser Gleichung durch geometrische
...
\\[0.2cm]
Setzen wir nun für $f(x)$ die Reihe $\sum\limits_{n=0}^\infty a_n \cdot x^n$ ein und fassen die die
