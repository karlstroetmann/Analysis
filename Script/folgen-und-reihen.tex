...
jetzt zu formalisieren, denn sonst können wir den Nachweis der \emph{Vollständigkeit} der reellen
...
Ein Paar $\pair(M_1,M_2)$ heißt \href{http://de.wikipedia.org/wiki/Dedekindscher_Schnitt}{\emph{Dedekindscher Schnitt}}
...
Dedekindscher Schnitt. \eox

Formal können wir nun die Menge der reellen Zahlen als die Menge aller Dedekindschen Schnitte definieren:
\\[0.2cm]
\hspace*{1.3cm}
$\mathbb{R} := \bigl\{ \pair(M_1,M_2) \in 2^\mathbb{Q} \times 2^\mathbb{Q} \mid \mbox{$\pair(M_1,M_2)$ ist eine Dedekindscher Schnitt}\bigr\}$.
\\[0.2cm]
...
lassen.  Zusätzlich ist zu klären, wie die Relation $<$ für zwei Dedekindsche Schnitte definiert
werden kann.  Aus Zeitgründen können wir diese Definitionen im Rahmen dieser Vorlesung nicht
präsentieren.  Für uns wichtig ist nur das folgende Ergebnis.
...
\item $M_1$ definieren wir als die Menge aller unteren Schranken der Folge $\folge{a_n}$
      \\[0.2cm]
      \hspace*{1.3cm}      
      $M_1 := \bigl\{ x \in \mathbb{Q} \mid \forall n \in \mathbb{N}: x \leq a_n \bigr\}$.
      \\[0.2cm]
...
\hspace*{1.3cm}
$ \begin{array}{lcl}
          M_2 & = & \mathbb{Q} \backslash M_1 \\[0.2cm]
              & = & \mathbb{Q} \backslash \bigl\{ x \in \mathbb{Q} \mid \forall n \in \mathbb{N}: x \leq a_n \bigr\} \\[0.2cm]
              & = & \bigl\{ x \in \mathbb{Q} \mid \neg \forall n \in \mathbb{N}: x \leq a_n \bigr\} \\[0.2cm]
              & = & \bigl\{ x \in \mathbb{Q} \mid \exists n \in \mathbb{N}: \neg x \leq a_n \bigr\} \\[0.2cm]
              & = & \bigl\{ x \in \mathbb{Q} \mid \exists n \in \mathbb{N}: a_n < x \bigr\}     
      \end{array}
      $
\\[0.2cm]
      Aus dieser Definition folgt unmittelbar, dass $M_1 \cup M_2 = \mathbb{Q}$ ist.
...
\\[0.2cm]
Folglich ist $\pair(M_1,M_2)$ ein Dedekind'scher Schnitt und wegen der Vollständigkeit der reellen
Zahlen gibt es eine Trennungs-Zahl $s$ mit der Eigenschaft, dass
\\[0.2cm]
\hspace*{1.3cm}
...
$ | a_n - s |  < \varepsilon $
\\[0.2cm]
für alle $n \geq n_0$ und damit ist bewiesen, dass $\lim\limits_{n\rightarrow\infty} a_n = s$
...
\exercise
Zeigen Sie unter Verwendung des Vollständigkeit der reellen Zahlen: Jede nichtleere nach unten beschränkte Menge
...
$ \begin{array}{lcl}
       ...
       $
\\[0.2cm]
       denn aus $n=2\cdot m+1$ und $n>K$ folgt $m>K_1$ und $n \geq K_2$.
\end{enumerate}
Damit ist der Beweis abgeschlossen. \hspace*{\fill} $\Box$ 
\vspace*{0.3cm}
...
\hspace*{1.3cm}
$ \forall \varepsilon \in \mathbb{R}_+: \exists K \in \mathbb{R}: \forall m,n \in \mathbb{N}: 
   m > K \wedge n \geq K \rightarrow \bigl| s_m - s_n \bigr| < \varepsilon.
$
\\[0.2cm]
