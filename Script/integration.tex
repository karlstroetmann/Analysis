...
$\ds \frac{dy}{\dr x} = f'(x)$
\\[0.2cm]
Wir rechnen nun mit dem Ausdruck $\ds\frac{dy}{\dr x}$ so, als ob es ein gewöhnlicher Bruch wäre und stellen die
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds\int g(y)\, dy = \int g\bigl(f(x)\bigr)\cdot f'(x)\, \dr x$
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds ds \approx \sqrt{h^2 + \bigl(f'(x) \cdot h\bigr)^2\,} = \sqrt{1 + \Bigl(\frac{d\/f}{\dr x}\Bigr)^2\,}\cdot h$.
\\[0.2cm]
Die Länge $l$ des gesamten Funktions-Graphen erhalten wir, wenn wir diesen Ausdruck von $a$ bis $b$
aufintegrieren: 
\\[0.2cm]
\hspace*{1.3cm}
\colorbox{orange}{\framebox{$\ds l = \dint{a}{b} \sqrt{1 + \Bigl(\frac{df}{\dr x}\Bigr)^2\,} \dr x$}}.
