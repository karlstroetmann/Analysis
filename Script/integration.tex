...
\begin{figure}[!h]
  \centering
   \epsfig{file=xhoch2int.eps,scale=0.4}
...
\begin{figure}[!t]
  \centering
   \epsfig{file=xhoch2RiemannRight.eps,scale=0.5}
   \caption{Die Fläche unter der Parabel $x \mapsto x^2$ im Intervall $[0,1]$.}
...
\begin{equation}
  \label{eq:intUnter}
  \int_a^b f^\uparrow_n(x) \,dx = \sum\limits_{i=1}^{n} f\bigl(a+(i-1)\cdot h\bigr)\cdot hw
...
\\[0.2cm]
\hspace*{1.3cm}
$\sum\limits_{i=0}^{n-1} f(a+i\cdot h)\cdot h \leq \int_a^b f(x) \,dx \leq \sum\limits_{i=1}^{n} f(a+i\cdot h)\cdot h$ 
\\[0.2cm]
...
$
\begin{array}{cl} 
    & \sum\limits_{i=1}^{n} f(a+i\cdot h)\cdot h - \sum\limits_{i=0}^{n-1} f(a+i\cdot h)\cdot h \\[0.4cm]
 =  &  f(a+n\cdot h)\cdot h - f(a)\cdot h  \\[0.1cm]
 =  &  \biggl(f\Bigl(a+n\cdot \bruch{b-a}{n}\Bigr) - f(a)\biggr)\cdot  \bruch{b-a}{n}  \\[0.3cm]
 =  &  \Bigl(f(a+ b-a) - f(a)\Bigr)\cdot  \bruch{b-a}{n}  \\[0.3cm]
 =  &  \Bigl(f(b) - f(a)\Bigr)\cdot  \bruch{b-a}{n}  \\[0.3cm]
...
\\[0.2cm]
existiert, so nennen wir $f$ \emph{integriebar} und definieren das \emph{Integral} von $f$
...
\item Linearität: Sind $f$ und $g$ zwei Funktionen, so dass
      das Integral über $f$ bzw.~$g$ in dem Intervall $[a,b]$ definiert ist
...
\item Monotonie:  Sind $f$ und $g$ zwei Funktionen, so dass
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $\Bigl(\forall x \in [a,b]: f(x) \leq g(x)\Bigr) \;\Rightarrow\;
...
\\[0.2cm]
\hspace*{1.3cm} $\int_a^b f(x) \,dx = f(\xi) \cdot  (b - a)$.
\end{Satz}
...
\Leftrightarrow & \displaystyle f\bigl(x_{min}\bigr) \cdot  (b-a) \leq \int_a^b f(x)\, dx \leq f\bigl(x_{max}\bigr)\cdot (b-a) \\[0.3cm]
\Leftrightarrow & \displaystyle f\bigl(x_{min}\bigr) \leq \frac{\normalsize\int_a^b f(x)\, dx}{b-a} \leq f\bigl(x_{max}\bigr) \\[0.3cm]
\end{array}
$
\\[0.2cm]
Die obige
Ungleichungskette zeigt, dass 
\\[0.2cm]
\hspace*{1.3cm}
 $\displaystyle \frac{\;\normalsize\int_a^b f(x)\, dx\;}{b-a} \in \bigl[f(x_{min}), f(x_{max})\bigr]$ \\[0.2cm]
gilt.  
...
\\[0.2cm]
\hspace*{1.3cm}
$f(\xi) = \displaystyle \frac{\;\int_a^b f(x)\, dx\;}{b-a}$
\\[0.2cm]
gilt und dass ist äquivalent zu 
\\[0.2cm]
\hspace*{1.3cm}
$f(\xi) \cdot  (b- a) = \int_a^b f(x)\, dx$. \hspace*{\fill} $\Box$
\vspace*{0.3cm}
...
\begin{array}[t]{cll}  
 ...
 = & \lim\limits_{h\rightarrow 0} \displaystyle\frac{\int_a^{x+h} f(t)\, dt - \int_a^x f(t)\, dt}{h} &
      \mbox{nach Definition von $F$} \\[0.3cm]
 = & \lim\limits_{h\rightarrow 0} \displaystyle\frac{1}{h}\int_{x}^{x+h} f(t)\, dt \\[0.5cm]
 = & \lim\limits_{h\rightarrow 0} \displaystyle\frac{1}{h} \cdot  (x+h - h) \cdot   f(\xi_h) &
      \mbox{nach dem Mittelwertsatz für ein $\xi_h\in[x, x+h]$} \\[0.5cm]
 = & \lim\limits_{h\rightarrow 0} \displaystyle f(\xi_h) \\[0.3cm]
 = & \displaystyle f(x) & \mbox{wegen $\xi_h \in [x, x+h]$}.
\end{array}
$
\\[0.3cm]
...
\hspace*{1.3cm} $\df F(x) = f(x)$
\\[0.2cm]
gilt.  In diesem Fall schreiben wir auch  $F(x) = \int f(x) \, dx$.
Der Ausdruck $\int f(x) \, dx$ wird auch als \emph{unbestimmtes Integral} bezeichnet.
\end{Definition}



Die Schreibweise $F(x) = \int f(x) \, dx$ ist problematisch, denn die Stamm-Funktion einer
...
\\[0.2cm]
\hspace*{1.3cm}
$G(x) := \int_a^x f(t)\, dt$ \quad für alle $x\in[a,b]$,
\\[0.2cm]
...
\begin{array}[t]{lcl}
  F(v) - F(u) & = & G(v) + c - \bigl(G(u) + c\bigr) \\[0.2cm]
              & = & G(v) - G(u)                     \\[0.2cm]
              & = & \int_a^v f(t)\, dt - \int_a^u f(t)\, dt \\[0.3cm]
              & = & \int_u^v f(t)\, dt. \hspace*{9cm} \Box  \\[0.3cm]              
\end{array}
$
...
\begin{equation}
  ...
  \int_u^v f'(x)\,dx = f(v) - f(u) = f(x)\Big|_u^v.    
\end{equation}
...
$
\begin{array}[t]{lcll}  
\df \bigl( x\cdot \ln(x) - x\bigr) & = & \df \bigl( x\cdot \ln(x) \bigr) - \df x \\[0.3cm] 
...
$
\\[0.3cm]
Damit ist gezeigt, dass $\int \ln(x)\;dx = x\cdot \ln(x) - x$ gilt.  Die übrigen Einträge der
...
\begin{table}[h]
  ...
  \begin{tabular}[t]{|l|l|}
    \hline
    Funktion $f(x)$ & Stamm-Funktion $\displaystyle \rule[-12pt]{0pt}{30pt}\int\!\! f(x) \, dx$ \\
    \hline
    \hline
...
\\[0.2cm]
\hspace*{1.3cm}
$\df \ln(-x) = -1 \cdot  \bruch{1}{-x} = \bruch{1}{x}$.
\\[0.2cm]
Diese beiden Gleichungen können wir zu 
\\[0.2cm]
\hspace*{1.3cm}
$\df \ln\bigl(|x|\bigr) = \bruch{1}{x}$
\\[0.2cm]
...
$
\begin{array}[t]{llcll}
            & \df h(f(x)) & = & f'(x) \cdot  h'(f(x)) & \mbox{Ketten-Regel} \\[0.3cm]
\Rightarrow & \bint\df h(f(x))\, dx & = & \bint f'(x) \cdot  h'(f(x))\, dx  \;+\; c & \mbox{Stamm-Funktion bilden} \\[0.3cm]
...
\begin{equation}
  \label{eq:subst}
\int_a^b f'(x) \cdot  g\bigl(f(x)\bigr)\, dx = \int_{f(a)}^{f(b)} g(x)\, dx  
\end{equation}
...
$\bruch{dy}{dx} = f'(x)$
\\[0.2cm]
Wir rechnen nun mit dem Ausdruck $\frac{dy}{dx}$ so, als ob es ein gewöhnlicher Bruch wäre und stellen die
...
\\[0.2cm]
\hspace*{1.3cm}
$\int g(y)\, dy = \int g\bigl(f(x)\bigr)\cdot f'(x)\, dx$
\\[0.2cm]
Das ist aber genau die Substitutions-Regel.

\exercise
Als Anwendung der Substitutions-Regel zeigen wir die Berechnung von $\int \tan(t)\,dt$:
\\[0.2cm]
\hspace*{1.3cm}
$
\begin{array}[t]{lcl}
...
Integral $\int \frac{\sin(t)}{\cos(t)}\,dt$ führen wir die Variablen-Transformation $x =
\cos(t)$ durch.  Dann gilt
\\[0.2cm]
\hspace*{1.3cm} $\bruch{dx}{dt} = -\sin(t) \;\Leftrightarrow\; dt = \bruch{dx}{-\sin(t)}$.
\\[0.2cm]
Ersetzen wir in dem Integral $\int \frac{\sin(t)}{\cos(t)}\,dt$ den Ausdruck $dt$ durch
$\frac{dx}{-\sin(t)}$ und $\cos(t)$ durch $x$, so erhalten wir
\\[0.2cm]
\hspace*{1.3cm} 
$\displaystyle \int \bruch{\sin(t)}{\cos(t)}\,dt \;=\; \int
\bruch{\sin(t)}{x}\cdot \bruch{dx}{-\sin(t)} \;=\; - \int \bruch{1}{x}\, dx = -\ln(|x|) =
-\ln\bigl(|\cos(t)|\bigr) $.  
\\[0.2cm]
Das Rechnen mit den \emph{infinitesimalen} Größen $dx$ und $dt$ ist
offensichtlich intuitiver als die formale Anwendung der Substitutions-Regel.
\pagebreak
...
\\[0.2cm]
mit  Hilfe der Substitutions-Regel.
...
$
\begin{array}[t]{llcll}
                & \df \Bigl(f(x)\cdot g(x)\Bigr)  & = & f'(x)\cdot  g(x) + f(x) \cdot  g'(x) &\mbox{(Umstellen)} \\[0.3cm]
\Leftrightarrow & f'(x) \cdot  g(x) & = & \df \Bigl(f(x)\cdot g(x)\Bigr) - f(x) \cdot  g'(x) &\mbox{(Stamm-Funktion)} \\[0.3cm]
\Leftrightarrow & \displaystyle \int f'(x)\cdot  g(x)\,dx & = & \bint\df \Bigl(f(x)\cdot g(x)\Bigr)\,dx - \int f(x) \cdot  g'(x)\,dx \\[0.3cm]
\end{array}
$
\\[0.2cm]
Die Stamm-Funktion von $\df\Bigl( f(x)\cdot g(x) \Bigr)$ ist natürlich $f(x)\cdot g(x)$, also haben wir
\begin{equation}
  \label{eq:partInt0}
 \int f'(x)\cdot  g(x)\,dx =  \displaystyle f(x)\cdot g(x) - \int f(x) \cdot  g'(x)\,dx 
\end{equation}
...
\begin{array}[t]{lcl}
 \bint\ln(x)\,dx & = &  \bint1\cdot \ln(x)\,dx \\[0.2cm]
                 & = &  x\cdot \ln(x) - \bint x\cdot \df \ln(x)\,dx \\[0.3cm]
...
\exercise
Berechnen Sie die Stamm-Funktion $\int \arctan(x)\,dx$.
...
Die Umkehr-Funktion von $f$ sei die Funktion $g:\bigl[f(a), f(b)\bigl]
\rightarrow\mathbb{R}$, für alle $x\in[a,b]$ gilt
also $g\bigl(f(x)\bigr) = x$.  Analog gilt dann für alle $y\in\bigl[f(a),f(b)\bigr]$
...
\item Die vertikal schraffierte Fläche ist die Fläche unter der Kurve $f(x)$ und ist
      ...
      \\[0.2cm]
      \hspace*{1.3cm} $\displaystyle \int_a^b f(x)\, dx$.
\item Die horizontal schraffierte Fläche ist die Fläche unter der Kurve $g(y)$ der
      ...
      \\[0.2cm]
      \hspace*{1.3cm} $\displaystyle \int_{f(a)}^{f(b)} g(y)\, dy$.
\end{enumerate}
...
\begin{figure}[!h]
  \centering
  \framebox{\epsfig{file=umkehr,scale=0.4}} 
...
\end{equation}
Als Beispiel betrachten wir die Funktion $f(x) = \ln(x)$ mit der Umkehr-Funktion 
$g(x) = \exp(x)$.  Wegen $\int \exp(x)\,dx = \exp(x)$ gilt 
\\[0.3cm]
\hspace*{1.3cm}
$\bint \ln(x) = x\cdot \ln(x) - \exp\bigl(\ln(x)\bigr) = x\cdot \ln(x) - x$.

\subsection{Beispiele}
Wir geben ein Paar Beispiele die zeigen, wie Integrale berechnet werden. 
Als erstes berechnen wir den Flächen-Inhalt eines Kreises.  Ein Kreis vom Radius 1 kann
nach dem Satz des Pythagoras 
in der Koordinaten-Ebene durch die Relation $x^2 + y^2 = 1$ dargestellt werden.
Lösen wir diese Gleichung nach $x$ auf, so wird der Kreis durch die beiden Funktion $x
\mapsto \sqrt{1 - x^2}$
und Funktion $x \mapsto -\sqrt{1 - x^2}$ für $x\in[-1,1]$ beschrieben.  Wir beschränken
...
\\[0.2cm]
\hspace*{1.3cm}
$\displaystyle I = \int_0^{\frac{\pi}{2}} \sqrt{1 - \sin^2(t)\,} \cdot \cos(t)\, dt = \int_0^{\frac{\pi}{2}} \cos(t) \cdot \cos(t)\, dt$.
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\displaystyle I = \sin(t)\cdot \cos(t)\bigg|_0^{\frac{\pi}{2}} + \int_0^{\frac{\pi}{2}}\sin(t)\cdot\sin(t)\,dt
   = \int_0^{\frac{\pi}{2}}\sin^2(t)\,dt$
\\[0.2cm]
Damit haben wir jetzt 
\\[0.2cm]
\hspace*{1.3cm}
$\displaystyle I = \int_0^{\frac{\pi}{2}}\cos^2(t)\,dt =  \int_0^{\frac{\pi}{2}}\sin^2(t)\,dt$
\\[0.2cm]
Daraus folgt \\
\hspace*{1.3cm}
$
\begin{array}[t]{llcl}
\Leftrightarrow &  2\cdot I & = & \displaystyle \int_0^{\frac{\pi}{2}}\cos^2(t)\,dt + \int_0^{\frac{\pi}{2}}\sin^2(t)\,dt \\[0.5cm]
\Leftrightarrow &  2\cdot I & = & \displaystyle \int_0^{\frac{\pi}{2}}\bigl(\cos^2(t)\,dt + \sin^2(t)\bigr)\,dt \\[0.5cm]
\Leftrightarrow &  2\cdot I & = & \displaystyle \int_0^{\frac{\pi}{2}} 1 \,dt \\[0.5cm]
\Leftrightarrow &  2\cdot I & = & \displaystyle t\; \Big|_0^{\frac{\pi}{2}}  \\[0.4cm]
...
\\[0.2cm] 
Nach Definition dee Ableitung von $f$ gilt für kleine Werte von $dx$
\\[0.2cm]
\hspace*{1.3cm}
$\bruch{f(x+dx) - f(x)}{x + dx - dx} \approx f'(x)$.
\\[0.2cm]
Also haben wir 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$ds \approx \sqrt{dx^2 + \bigl(f'(x) \cdot dx\bigr)^2\,} = \sqrt{1 + \Bigl(\bruch{df}{dx}\Bigr)^2\,}\cdot dx$.
\\[0.2cm]
Die Länge $l$ des gesamten Funktions-Graphen erhalten wir, wenn wir diesen Ausdruck von $a$ bis $b$
aufintegrieren: 
\\[0.2cm]
\hspace*{1.3cm}
$l = \dint{a}{b} \sqrt{1 + \Bigl(\bruch{df}{dx}\Bigr)^2\,} dx$.
...
\\[0.2cm]
\hspace*{1.3cm}
$\bruch{df}{dx} = \frac{1}{2} \cdot \bruch{-2 \cdot x}{\sqrt{1 - x^2}} = 
                  \bruch{- x}{\sqrt{1 - x^2}}
$.
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\displaystyle \int_a^b f(x)\, dx := \lim\limits_{h\rightarrow 0 \atop h > 0} \int_{a+h}^b f(x)\, dx$.
\\[0.2cm]
Wir betrachten als Beispiel das Integral der Funktion $x \mapsto \frac{1}{\sqrt{x}}$ in
dem Intervall $[0,1]$.  Es gilt
\\[0.2cm]
\hspace*{1.3cm}
$\displaystyle \int_0^1 \bruch{1}{\sqrt{x}}\, dx 
...
\\[0.2cm]
\hspace*{1.3cm}
$\displaystyle \int_a^\infty f(t)\, dt := \lim\limits_{x\rightarrow \infty} \int_{a}^x f(t)\, dt$.
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\displaystyle \lim\limits_{n \rightarrow \infty} \sum\limits_{i=1}^n f(i)$ existiert \quad g.d.w. \quad
$\displaystyle \lim\limits_{x \rightarrow \infty} \int\limits_1^x f(t)\,dt$ existiert.  
\end{Satz}
...
\\
\hspace*{1.3cm}
$\displaystyle \biggl(\sum\limits_{i=1}^{n}  f(i)\biggr)_{n\in\mathbb{N}}$ 
\\ 
monoton und
...
$
\begin{array}[t]{clcl}  
  &I(\alpha) & := & \displaystyle \int_{1}^\infty \bruch{1}{t^\alpha}\, dt  \\[0.3cm]
= & \displaystyle \lim\limits_{x \rightarrow \infty} \int_1^x \bruch{1}{t^\alpha}\, dt 
& = & \displaystyle \lim\limits_{x \rightarrow \infty} \bruch{t^{1-\alpha}}{1-\alpha} \,\bigg|_1^x \\[0.5cm]
= & \displaystyle \lim\limits_{x \rightarrow \infty} \bruch{x^{1-\alpha}-1}{1-\alpha}
\end{array}
$
\\[0.3cm]
...
\item Falls $\alpha < 1$ ist, haben wir $1-\alpha>0$ und damit gilt
      \\[0.2cm]
      \hspace*{1.3cm}
      $\displaystyle I(\alpha) = \lim\limits_{x \rightarrow \infty} \bruch{x^{1-\alpha}-1}{1-\alpha} = \infty$
      \\[0.2cm]
...
\item Falls $\alpha > 1$ ist, haben wir $1-\alpha<0$ und damit gilt
      \\[0.2cm]
      \hspace*{1.3cm}
      $\displaystyle I(\alpha) = \lim\limits_{x \rightarrow \infty} \bruch{x^{1-\alpha}-1}{1-\alpha} = \bruch{1}{\alpha-1}$
      \\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\displaystyle \sum\limits_{n=1}^\infty \bruch{1}{n\cdot (n+1)}$ 
\\[0.2cm]
konvergiert.
\pagebreak
...
$$
\begin{array}[t]{lcl}
 \dint{a}{b} f(x)\,dx & \approx & \displaystyle
...
\begin{figure}[!h]
  \centering
   \epsfig{file=trapezoidal.eps,scale=0.6}
...
\begin{Satz}
  ...
  \\[0.2cm]
  \hspace*{1.3cm}
  $\displaystyle \left| \int_a^b f(x)\,dx - I_{\mbox{\scriptsize Trapez}} \right| \;\leq\; 
...
$
\begin{array}[t]{cl}
      & \displaystyle  \left|\int_{x_{i-1}}^{x_i} f(x)\,dx - \int_{x_{i-1}}^{x_i}p(x)\,dx\right| \\[0.5cm]
 \leq & \displaystyle  \int_{x_{i-1}}^{x_i} \bigl|f(x)\,dx - p(x)\bigr|\,dx \\[0.5cm]
 \leq & \displaystyle \bruch{K}{2} \cdot  \int_{x_{i-1}}^{x_i} \bigl|(x-x_{i-1})\cdot (x-x_i)\bigr|\,dx  \\[0.5cm]
 \leq & \displaystyle -\bruch{K}{2} \cdot  \int_{x_{i-1}}^{x_i} (x-x_{i-1})\cdot (x-x_i)\,dx  \\[0.5cm]
 =    & \displaystyle -\bruch{K}{2} \cdot  \bruch{-1}{6} \cdot  (x_i-x_{i-1})^3\\[0.3cm]
 =    & \displaystyle \bruch{K}{12} \cdot  (x_i-x_{i-1})^3
\end{array}
$
\\[0.2cm]
...
$
\begin{array}[t]{lcl}  
\displaystyle \left| \int_a^b f(x)\,dx - I_{\mbox{\scriptsize Trapez}} \right|
& \leq & \displaystyle \sum\limits_{i=1}^n \bruch{K}{12} \cdot  (x_i-x_{i-1})^3 \\[0.5cm]
& \leq & \displaystyle \sum\limits_{i=1}^n \bruch{K}{12} \cdot  \Bigl(\frac{b-a}{n}\Bigr)^3 \\[0.5cm]
& \leq & \displaystyle n \cdot  \bruch{K}{12} \cdot  \Bigl(\frac{b-a}{n}\Bigr)^3  \\[0.5cm]
& \leq & \displaystyle \bruch{K}{12} \cdot  \frac{(b-a)^3}{n^2} \\[0.5cm]
\end{array}
$
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\displaystyle \int_{x_{i-1}}^{x_{i+1}} p(x)\,dx = \bruch{1}{6}\cdot \bigl(f(x_{i-1}) + 4 \cdot  f(x_i) + f(x_{i+1})\bigr)\cdot (x_{i+1}-x_{i-1})$
\\[0.2cm]
...
\begin{array}[t]{lcl}
 \displaystyle \int_a^b f(x)\,dx 
& \approx & \displaystyle\frac{h}{3}\cdot \sum\limits_{i=0}^{(n-1)/2} \Bigl( f(x_{2\cdot i}) + 4\cdot f(x_{2\cdot i+1}) + f(x_{2\cdot i+2}) \Bigr) \\[0.3cm]
& = & \displaystyle\frac{h}{3}\cdot \biggl(f(x_0) \;+\; 4\cdot \sum\limits_{i=0}^{(n-1)/2} f(x_{2\cdot i+1}) \;+\;
...
\\[0.2cm]
\hspace*{1.3cm}
 $\displaystyle \sum\limits_{k=1}^\infty \bruch{1}{k^2}$.
\\[0.2cm]
...
\item Approximieren Sie die Rest-Summe $\sum\limits_{k=n}^\infty \bruch{1}{k^2}$ 
      ...
      \textbf{Hinweis}: Es gilt $\displaystyle f(k) = \int_{k-\frac{1}{2}}^{k+\frac{1}{2}} f(k) \, dt \approx \int_{k-\frac{1}{2}}^{k+\frac{1}{2}} f(t) \, dt$.
