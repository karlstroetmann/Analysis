...
\example  Für die Folge $\folge{n}$ gilt offenbar 
\\[0.2cm]
\hspace*{1.3cm}      
...
äquivalent ist. Diese Definition trägt den Namen \emph{$\varepsilon$-$\delta$-Definition der Stetigkeit} und
Funktionen, die nach dieser Definition stetig sind, heißen $\varepsilon$-$\delta$-stetig.
...
initialisieren diese Werte mit $1.0/0.0 = \infty$.
Falls wir in Zeile 18 feststellen, dass der Wert von $a_n = a_{n-2}$ ist, dann teilen wir den Wert
$f(a_n)$ durch zwei.  Analog testen wir in Zeile 13, ob $b_n = b_{n-2}$ ist und teilen
gegebenenfalls $f(b_n)$ durch zwei.
...
\begin{Verbatim}[ frame         = lines, 
    ...
    illinois := procedure(f, a, b, n) {
        ...
        k := 1;
        [ oldA1, oldB1 ] := [ 1.0/0.0, 1.0/0.0 ];
        [ oldA2, oldB2 ] := [ 1.0/0.0, 1.0/0.0 ];
        while (k <= n) {
            c  := (fb * a - fa * b) / (fb - fa);
            fc := f(c);
            if (fc < 0) {
