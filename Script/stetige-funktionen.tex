...
ist, ist aber die Forderung, dass die Funktion $f$ glatt ist, denn wenn wir beispielsweise die Funktion
\\[0.2cm]
\hspace*{1.3cm}
...
$
\\[0.2cm]
betrachten, so haben wir zwar  $g(-1) = -1$ und $g(1) = 1$, aber es gibt kein $x \in [-1,1]$, 
...
Attributs \emph{glatt} der Begriff der \emph{Stetigkeit} verwendet.  Um diesen Begriff einführen zu können,
...
\emph{die Folge $\folge{x_n}$ in $D$ liegt}, wenn für alle $n \in \mathbb{N}$  gilt $x_n \in D$.
...
\begin{Definition}[Stetigkeit]
  ...
  wenn gilt: \\[0.1cm]
  \hspace*{1.3cm}      
  $\lim\limits_{x\rightarrow \widehat{x}} f(x) = f(\widehat{x})$.
\end{Definition}
...
\item Die Funktion $\textsl{sign}:\mathbb{R} \rightarrow \mathbb{R}$ sei durch
      ...
      $\folge{\frac{1}{n+1}}$ gilt
      \\[0.1cm]
      \hspace*{1.3cm}      
      $\lim\limits_{n\rightarrow\infty} \frac{1}{n+1} = 0$, \quad aber \quad
      $\lim\limits_{n\rightarrow\infty} \textsl{sign}\left(\frac{1}{n+1}\right) =
...
Wir sagen, dass eine Folge
 $\folge{x_n}$ gegen Unendlich konvergiert und schreiben 
      \\[0.1cm]
      \hspace*{1.3cm}      
...
\noindent
\textbf{Beispiel}:  Für die Folge $\folge{n}$ gilt offenbar 
      \\[0.1cm]
      \hspace*{1.3cm}      
...
\\[0.1cm]
\hspace*{1.3cm}
$\lim\limits_{n\rightarrow\infty} \bruch{1}{x_n} = 0$.
\\[0.1cm]
...
$c := \frac{1}{\varepsilon}$ definieren, dann finden wir eine Zahl $K$, so dass für alle
natürlichen Zahlen $n$, die
größer als $K$ sind, die Ungleichung  
\\[0.1cm] \hspace*{1.3cm} $x_n > c$, \quad also $x_n > \frac{1}{\varepsilon}$ \\[0.1cm]
...
äquivalent ist. Diese Definition trägt den Namen $\varepsilon$-$\delta$-Definition der
Steigkeit.  Wir formulieren die Äquivalenz dieser Definition mit der oben gegebenen
Definition als Satz.

\begin{Satz} 
  Eine Funktion $f:D \rightarrow \mathbb{R}$ ist genau dann stetig im Punkt
  $\widehat{x}$, wenn gilt: 
  \\[0.1cm]
  \hspace*{1.3cm}
...
Eine Funktion $f:\mathbb{D} \rightarrow \mathbb{R}$ heißt \emph{stetig} genau dann, wenn
...
\emph{Bisektions-Verfahren} bezeichnet.  Das Verfahren folgt dem Pradigma 
...
\item[I.S.:] $n \mapsto n+1$
      ...
      \\[0.1cm]
      \hspace*{1.3cm} $c_n := \frac{1}{2} \cdot (a_n + b_n)$. \\[0.1cm]
...
\item $b_n \geq b_{n+1}$, \quad die Folge $\folge{b_n}$ ist also monoton
      fallend.
\item $a_n \leq b_n$.
\item $b_n - a_n = \left(\frac{1}{2}\right)^n \cdot (b - a)$.
\end{enumerate}
...
\item[I.S.:] $n \mapsto n+1$.  
      ...
      Wir setzen $c_n := \frac{1}{2} \cdot (a_n + b_n)$ und führen eine 
      ...
      \item $f(c_n) > 0$.  Dann gilt $a_{n+1} = a_n$ und 
            ...
            $
            \begin{array}[t]{lcl}
              b_{n+1} - a_{n+1} & = & \frac{1}{2}\cdot(a_n + b_n) - a_n \\[0.1cm]
                                & = & \frac{1}{2}\cdot(b_n - a_n)       \\[0.1cm]  
                  & \stackrel{IV}{=} & \frac{1}{2}\cdot\left(\frac{1}{2}\right)^n\cdot(b - a) \\[0.1cm]  
                                & = & \left(\frac{1}{2}\right)^{n+1}\cdot(b - a) \\[0.1cm]  
            \end{array}
            $
      \item $f(c_n) \leq 0$.  Jetzt gilt $a_{n+1} = c_n = \frac{1}{2}\cdot(a_n + b_n)$ und 
            ...
            $
            \begin{array}[t]{lcl}
              b_{n+1} - a_{n+1} & = & b_n - \frac{1}{2}\cdot(a_n + b_n) \\[0.1cm]
                                & = & \frac{1}{2}\cdot(b_n - a_n)       \\[0.1cm]  
                  & \stackrel{IV}{=} & \frac{1}{2}\cdot\left(\frac{1}{2}\right)^n\cdot(b - a) \\[0.1cm]  
                                & = & \left(\frac{1}{2}\right)^{n+1}\cdot(b - a) \\[0.1cm]  
            \end{array}
...
\hspace*{1.3cm} $a = a_0 \leq a_1 \leq \cdots a_{n-1} \leq a_n \leq b_n$, 
                also gilt $a \leq b_n$ für alle $n\in\mathbb{N}$ \\[0.1cm]
...
\Leftrightarrow & n \geq - \frac{1}{\ln(2)} \cdot \ln\Bigl(\bruch{\varepsilon}{b - a}\Bigr) \\[0.4cm]
\Leftrightarrow & n \geq \frac{1}{\ln(2)} \cdot \ln\Bigl(\bruch{b - a}{\varepsilon}\Bigr)
\end{array}
$
\\[0.4cm]
...
\\[0.1cm]
Diese Forderung ermöglicht eine etwas einfachere Implementierung.

Das letzte Argument der Methode \textsl{findZero}() gibt nun die gewünschte Zahl der
Iterationen an.  Dieses Argument ist erforderlich, denn die Methode selber liefert uns
leider keinen Hinweis, wie gut die bisher berechnete Näherung ist.
Das liegt daran, dass im allgemeinen die Folgen $a_n$ und $b_n$ nicht gegen den selben
Grenzwert konvergieren.

Bei der Rückgabe des berechneten Wertes in Zeile 16 ist es erforderlich, die Beträge der
Funktionswerte 
...
ein Verfahren haben, dass dem Bisektions-Verfahren überlegen ist.  Die nächste Aufgabe
...
behoben, aber dort kann es in ungünstigen Fällen passieren, dass das Verfahren überhaupt nicht
...
\item[I.S.:] $n \mapsto n+1$.  Wir definieren ähnlich wie bei der Regula Falsi den Wert $c_n$ als \\[0.2cm]
      ...
      $
      \\[0.2cm]
      Falls wir nun feststellen, dass $b_{n+1} = b_n$ gilt, so hat sich der Wert der rechten
      Intervall-Grenze nicht geändert.  Wir wollen diesen Wert daher beim nächsten Iterations-Schritt
      schwächer gewichten und setzen deshalb in diesem Fall
      \\[0.2cm]
      \hspace*{1.3cm}
      $\beta_{n+1} = \bruch{1}{2} \cdot \beta_n$ \quad und \quad $\alpha_{n+1} := 1$.
      \\[0.2cm]
      Ist umgekehrt $a_{n+1} = a_n$, so hat sich der Wert der linken
...
\end{enumerate}
Die Umsetzung dieses Verfahrens sehen Sie in Abbildung \ref{fig:illinois.stlx}.
...
\\[0.3cm]
Wir bezeichnen den Wert $\frac{d\,f}{dx}(\widehat{x})$ als die \emph{Ableitung} der
Funktion $f$ an der Stelle $\widehat{x}$.  Gelegentlich werden wir auch die Schreibweise 
$f'(\widehat{x})$ für die Ableitung verwenden.
\end{Definition}

\noindent
\textbf{Bemerkung}: Beachten Sie, dass wir in der obigen Definition den Ausdruck
\\[0.3cm]
\hspace*{1.3cm} $\bruch{f(\widehat{x} + h) - f(\widehat{x})}{h}$ \\[0.3cm]
als Funktion von $h$ auffassen.  Dieser Ausdruck wird auch als \emph{Differenzial-Quotient}
...
\noindent
\textbf{Bemerkung}: Falls die Funktion $f$ im Punkt $\widehat{x}$ differenzierbar ist,
...
\begin{Satz}[Ketten-Regel] 
  ...
  $\widehat{y} = f(\widehat{x})$.  Dann ist auch
  die Funktion $g \circ f := \bigr(x \mapsto g(f(x))\bigr)$ im Punkt $\widehat{x}$ 
  differenzierbar und es gilt 
  \\[0.3cm]
  \hspace*{1.3cm}
...
\end{enumerate}
Damit finden wir für den Differenzial-Quotienten der Funktion $g \circ f$ im Punkt
$\widehat{x}$: \\[0.3cm]
\hspace*{1.3cm}
$
\begin{array}[t]{lcl}
...
& = & 0 \cdot \bigl(f'(\widehat{x}) + 0\bigr) \\[0.1cm]
& = & 0
\end{array}
$
\\[0.3cm]
...
$
\\[0.3cm]
Die linke Seite der Gleichung stellt den Differenzial-Quotienten der Funktion $g\circ f$
...
\item Die Ableitung des Cosinus könnte in analoger Weise berechnet werden, es ist aber 
      ...
      $
      \begin{array}[t]{lcll}
      \df\cos(x) & = & \df\sin\bigl(\frac{\pi}{2}- x\bigr) \\[0.3cm]
                 & = & \cos(\frac{\pi}{2}- x\bigr) \cdot \df(\frac{\pi}{2}- x\bigr) & \mbox{nach der Ketten-Regel} \\[0.3cm]
                 & = & \sin(x) \cdot (-1) \\[0.3cm]
                 & = & -\,\sin(x). \\[0.3cm]
      \end{array}$
\item Jetzt kann die Ableitung der Tangens-Funktion über die Quotienten-Regel berechnet 
      ...
      $\displaystyle
      \begin{array}[t]{lcl}
      \frac{d\;}{dx} \tan(x) & = & \frac{d\;}{dx} \left(\bruch{\sin(x)}{\cos(x)}\right) \\[0.3cm]
...
\item Die Ableitung der Arcus-Tangens-Funktion kann nun mit dem selben Trick berechnet werden,
      ...
      \\[0.3cm]
      \hspace*{1.3cm}
      $\frac{d\;}{dx} \arctan\bigl(\tan(x)\bigr) \cdot \bruch{1}{\cos^2(x)} = 1$.
      \\[0.3cm]
      Multiplikation mit $\cos^2(x)$ ergibt
      \\[0.3cm]
      \hspace*{1.3cm}
      $\frac{d\;}{dx} \arctan\bigl(\tan(x)\bigr) = \cos^2(x)$.
      \\[0.3cm]
...
Zeigen Sie 
\\[0.3cm]
\hspace*{1.3cm} $\frac{d\;}{dx} \arcsin(x) = \bruch{1}{\sqrt{1 - x^2}}$.
...
\begin{Definition}[lokales Maximum]
  ...
  \\[0.1cm]
  \hspace*{1.3cm}
  $\exists \varepsilon \in \mathbb{R}_+: \forall x \in \mathbb{R}: |x - \bar{x}| < \varepsilon \rightarrow f(x) \leq f(\bar{x})$.
\end{Definition}
...
\begin{Satz}[Pierre de Fermat, 160?--1665]
...
\proof
Wir betrachten zunächst die Folge $\folge{\bar{x} + \frac{1}{n+1}}$.
O.B.d.A. sei $\varepsilon$ so klein gewählt, dass 
\\[0.1cm]
\hspace*{1.3cm}
$\forall x \in \mathbb{R} : |x - \bar{x}| < \varepsilon \rightarrow f(x) \leq f(\bar{x})$
\\[0.1cm]
gilt.  Wenn $n>\frac{1}{\varepsilon}$ liegt daher  $\bar{x} + \frac{1}{n+1}$ in der
$\varepsilon$-Umgebung von $\bar{x}$.  Daher gilt für alle $n > \frac{1}{\varepsilon}$
\\[0.3cm]
\hspace*{1.3cm} $f\bigl(\bar{x} + \frac{1}{n+1}\bigr) \leq f(\bar{x})$. 
\\[0.3cm]
Damit gilt für den Differenzial-Quotienten
\\[0.3cm]
\hspace*{1.3cm}
$\bruch{f\Bigl(\bar{x} + \frac{1}{n+1}\Bigr) - f(\bar{x})}{\bar{x} + \frac{1}{n+1} - \bar{x}} \;=\;
 (n+1) \cdot \left(f\Bigl(\bar{x} + \frac{1}{n+1}\Bigr) - f(\bar{x})\right) \leq 0$. 
\\[0.3cm]
Da wir vorausgesetzt haben, dass die Funktion $f$ im Punkt $\bar{x}$ differenzierbar ist,
gilt 
\\[0.3cm]
\hspace*{1.3cm}
$\df f(\bar{x}) = \lim\limits_{n \rightarrow \infty} \bruch{f\Bigl(\bar{x} + \frac{1}{n+1}\Bigr) - f(\bar{x})}{\bar{x} + \frac{1}{n+1} - \bar{x}} \leq 0$.
\\[0.3cm]
Wir betrachten nun die Folge $\folge{\bar{x} - \frac{1}{n+1}}$.
...
\hspace*{1.3cm}
$\bruch{f\Bigl(\bar{x} + \frac{1}{n+1}\Bigr) - f(\bar{x})}{\bar{x} - \frac{1}{n+1} - \bar{x}} \;=\;
 - (n+1) \cdot \left(f\Bigl(\bar{x} - \frac{1}{n+1}\Bigr) - f(\bar{x})\right) \geq 0$. 
\\[0.3cm]
Da wir vorausgesetzt haben, dass die Funktion $f$ im Punkt $\bar{x}$ differenzierbar ist,
gilt 
\\[0.3cm]
\hspace*{1.3cm}
$\df f(\bar{x}) = \lim\limits_{n \rightarrow \infty} \bruch{f\Bigl(\bar{x} - \frac{1}{n+1}\Bigr) - f(\bar{x})}{\bar{x} - \frac{1}{n+1} - \bar{x}} \geq 0$.
\\[0.3cm]
...
\end{Satz}
Aus Zeitgründen verzichten wir auf einen Beweis des letzten Satzes.

\begin{Satz}[Michel Rolle, 1652 -- 1719]
  Ist die Funktion $f:[a,b]\rightarrow \mathbb{R}$ differenzierbar und gilt $f(a) = f(b)$, dann gibt es ein
...
\begin{Satz}[Mittelwert-Satz der Differenzial-Rechnung, Augustin Louis Cauchy, 1789--1857]
...
\\[0.3cm]
Die Funktion $g$ ist offenbar differenzierbar und es gilt 
\\[0.3cm]
\hspace*{1.3cm}
...
\\[0.3cm]
Damit erfüllt die Funktion $g$ die Voraussetzung des Satzes von Rolle.  Also gibt es ein
$c \in (a,b)$, so dass 
\\[0.3cm]
\hspace*{1.3cm}
$\df g(c) = 0$
...
\\[0.2cm]
und wählen Sie $\alpha$ und $\beta$ geeignet.
...
\begin{array}[b]{lcl}
    ...
    \lim\limits_{h \rightarrow 0} \bruch{f(c) + h\cdot f'(c) + r_1(h)}{g(c) + h\cdot g'(c) + r_2(h)} \\[0.5cm]
&=& \lim\limits_{h \rightarrow 0} \bruch{h\cdot f'(c) + r_1(h)}{h\cdot g'(c) + r_2(h)} \\[0.5cm]
&=& \lim\limits_{h \rightarrow 0} \bruch{f'(c) + \frac{r_1(h)}{h}}{g'(c) + \frac{r_2(h)}{h}} \\[0.9cm]
...
\noindent
\textbf{Beispiel}: Mit dem Satz von L'H\^opital können wir nun den Grenzwert
$\lim\limits_{x \rightarrow 0} \frac{\sin(x)}{x}$ noch einmal berechnen: 
\\[0.3cm]
\hspace*{1.3cm} $\lim\limits_{x \rightarrow 0} \frac{\sin(x)}{x} = \lim\limits_{x \rightarrow 0}
...
\\[0.3cm]
\hspace*{1.3cm}
$\lim\limits_{x \rightarrow \infty} \frac{x}{\exp(x)} = \lim\limits_{x \rightarrow \infty} \frac{1}{\exp(x)} = 0$.
\\[0.3cm]
Der Satz von L'H\^opital läßt sich iteriert anwenden.  Beispielsweise gilt
\\[0.3cm]
\hspace*{1.3cm}
$\lim\limits_{x \rightarrow \infty} \frac{x^2}{\exp(x)} = \lim\limits_{x \rightarrow \infty} \frac{2\cdot x}{\exp(x)} =\lim\limits_{x \rightarrow \infty} \frac{2}{\exp(x)} = 0$.
\vspace*{0.3cm}
...
\begin{Definition}[monoton]
  ...
  \\[0.2cm]
  gilt.  Analog ist $f$ \emph{streng monoton fallend}, falls die Bedingung
  \\[0.2cm]
  \hspace*{1.3cm}
  $\forall x,y \in D: x < y \rightarrow f(x) > f(y)$
  \\[0.2cm]
...
\noindent
\textbf{Beweis}:  Wir nehmen zunächst an, dass $f$ monoton steigend ist und zeigen, dass
...
\\[0.3cm]
Wir zeigen, dass der Differenzial-Quotient 
\\[0.3cm]
...
\item Fall: $h < 0$.
      ...
      \hspace*{1.3cm} $\bruch{f(x+h) - f(x)}{h} \geq 0$.
\end{enumerate}
Da der Differenzial-Quotient in jedem Fall größer-gleich $0$ ist, muss $f'(x) \geq 0$
gelten.
\vspace*{0.2cm}

Wir nehmen nun an, dass für alle $x\in D$ die Ungleichung $f'(x) \geq 0$ gilt und
...
\\[0.1cm]
Nach dem Mittelwert-Satz der Differenzial-Rechnung gibt es dann ein $z\in[x,y]$, so dass
\\[0.3cm]
...
< \varepsilon = f''(x_0)$.
\\[0.2cm]
gilt.  Daraus folgt, dass für alle $x \in \mathbb{R}$ mit $|x - x_0| < \delta$ die
Ungleichung
\\[0.2cm]
...
streng monoton wachsend und für $x > x_0$ streng monoton fallend.  Dann muss $f$ aber ein Maximum in
$x_0$ haben. \qed
...
$f'(x_0) = 0 \wedge f''(x_0) > 0$.
\\[0.2cm]
erfüllt ist, dann hat die Funktion an der Stelle $x_0$ ein strenges lokales Minimum.
...
\item[``$\Leftarrow$'':]  Es seien $x_1$, $x_2$ und $t \in [0,1]$ gegeben.  O.B.d.A. sei weiter
  ...
  Differenzial-Rechnung gibt es jeweils ein $c_1 \in [x_1,x_0]$ und ein $c_2 \in [x_0,x_2]$, so dass
  \\[0.2cm]
  \hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$f^{(n)}(x) = 0$ \quad für alle $x \in \mathbb{R}$ ist.
\vspace*{0.3cm}
...
$\exp(x) \cdot \exp(-x) = 1$ 
\\[0.2cm]
gilt.  Folgern Sie daraus, dass die Exponential-Funktion keine Nulstelle hat.
...
\\[0.2cm]
\hspace*{1.3cm}
$f'(x) = \lambda \cdot f(x)$ \quad für ein $c \in \mathbb{R}$.
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$e^m = e^{-n} = \bruch{1}{e^n} = \bruch{1}{\exp(n)} = \exp(-n) = \exp(m)$.
\\[0.2cm]
Ist nun $\frac{p}{q} \in \mathbb{Q}$, wobei $p \in \mathbb{Z}$ und $q \in \mathbb{N}$ gilt, so haben wir 
...
\\[0.2cm]
\hspace*{1.3cm}
$\left(\exp\Bigl(\frac{p}{q}\Bigr)\right)^q = \exp\Bigl(q \cdot \frac{p}{q}\Bigr) = \exp(p)$.
\\[0.2cm]
...
$e^r = \exp(r)$
\\[0.2cm]
gezeigt.  Daher macht es Sinn, für beliebige reelle Zahlen $x \in \mathbb{R}$ den
Wert $e^x$ als
\\[0.2cm]
\hspace*{1.3cm}
$e^x := \exp(x)$
