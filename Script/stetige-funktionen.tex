...
\item Das letzte Beispiel lässt sich verallgemeinern: Die Funktionen
...
\hspace*{1.3cm} $a = a_1 \leq \cdots \leq a_{n-1} \leq a_n \leq b_n$, \quad
                also gilt $a \leq b_n$ für alle $n\in\mathbb{N}$.
 \\[0.2cm]
...
\\[0.2cm]
Dieser Satz lässt sich beweisen, indem wir die Funktion $g$ als
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
und wir können auf die Funktion $g$ den von uns bewiesen Zwischenwert-Satz anwenden.  Damit finden
eine Zahl $\bar{x} \in [a,b]$, für die $g(\bar{x}) = 0$ gilt.  Also haben wir
\\[0.2cm]
\hspace*{1.3cm}
$f(\bar{x}) -c = 0$
\\[0.2cm]
...
\exercise
Es war einmal ein buddhistischer Mönch, der einmal im Jahr zur Meditation auf einen Berg stieg.
Er begann seinen Weg um 6:00 Uhr morgens und erreichte den Gipfel um 18:00 Uhr abends.  Nachdem er
die ganze Nacht meditiert hatte, begann er am nächsten Morgen um 6:00 mit dem Abstieg, der bis 18:00
abends dauerte.  Beim Abstieg nahm der Mönch den selben Weg wie beim Aufstieg.  Zeigen Sie, dass es
eine Tageszeit $t$ gibt, an welcher der Mönch beim Abstieg am selben Ort ist wie beim Aufstieg. 
\eox
...
\noindent
Das im Beweis des letzten Satzes beschriebene Intervall-Halbierungs-Verfahren lässt sich
...
die Voraussetzungen des Zwischenwert-Satzes nicht erfüllt und das Bisektions-Verfahren lässt sich nicht anwenden.
...
werden, so muss die Zahl $n$ der Iterationen so gewählt werden, dass 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Damit ist klar, dass wir 31 Iterationen des Verfahrens benötigen um die geforderte
...
mehr als 10 mal so groß wie der Wert an der rechten Intervall-Grenze.  Folglich liegt es
...
Funktionswerte zu gewichten, denn die werden um so größer, je kleiner der Funktionswert
...
\item[I.S.:] $n \mapsto n+1$
      ...
      $
\end{enumerate}
Ähnlich wie beim Beweis des Zwischenwert-Satzes lässt sich zeigen, dass die Folge
...
$a_n$ und $b_n$ gegen den gleichen Grenzwert konvergieren!  Es lässt sich lediglich zeigen,
...
\item In Zeile 6 berechnen wir \texttt{c} nun nach der Formel
      ...
      \\[0.2cm]
      verwendet.
\item Bei der Rückgabe des berechneten Wertes in Zeile 14 bzw.~16 ist es erforderlich, die Beträge der
...
dass wir bereits im 9-ten Schritt dieselbe Genauigkeit erreicht haben, für die wir mit
...
\item[I.S.:] $n \mapsto n + 1$.
    ...
    $
      ...
      Daher gilt 
      \\[0.2cm]
      \hspace*{1.3cm}  $a_{n+1} \leq a_{n+1}' = c_n' \leq 10^{-3} \cdot n$ und $b_{n+1} = b_n = 10$.
      \qed
\end{enumerate}
...
mit der $x$-Achse bestimmen.  Das führt auf dieselbe Formel wie bei der Regula-Falsi, wir
setzen nämlich
\\[0.2cm]
\hspace*{1.3cm}
...
\item[I.A.:] $n=1$.  Wir setzen 
             ...
             \\[0.2cm]
             Die Werte $\alpha_1$ und $\beta_1$ sind dabei Gewichtungs-Faktoren, die wir später benötigen.
\item[I.S.:] $n \mapsto n+1$.  Wir definieren ähnlich wie bei der Regula Falsi den Wert $c_n$ als \\[0.2cm]
      ...
      Die Werte für $a_{n+1}$ und $b_{n+1}$ werden durch dieselbe Fall-Unterscheidung wie bei der Regula
      Falsi festgelegt:
      \\[0.2cm]
      \hspace*{1.3cm}
      $\pair(a_{n+1},b_{n+1}) := 
