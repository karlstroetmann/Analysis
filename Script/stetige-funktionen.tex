...
wir sie zeichnen k\"onnen, ohne dabei den Stift absetzen zu m\"ussen.  F\"ur eine glatte Funktion soll au\3erdem der
...
\begin{Definition}[Grenzwert]
  ...
  Weiter sei $\widehat{x} \in \mathbb{R}$ und  $\lambda\in \mathbb{R}$.  Au\3erdem gebe es
...
gr\"o\3er als $K$ sind, die Ungleichung 
\\[0.2cm]
...
und Funktionen, die nach dieser Definition stetig sind, hei\3en $\varepsilon$-$\delta$-stetig.
...
Eine Funktion $f:D \rightarrow \mathbb{R}$ hei\3t \emph{stetig} genau dann, wenn
...
ohne gro\3e M\"uhe  implementieren.  Abbildung \ref{fig:bisection.setlx}
...
Beim Bisektions-Verfahren wird das Interval in jedem Schritt in zwei gleich gro\3e Teile
...
nicht ber\"ucksichtigt.  Es liegt nahe, die Betr\"age der Funktionswerte in die Formel mit einflie\3en zu
...
mehr als 10 mal so gro\3 wie der Wert an der rechten Intervall-Grenze.  Folglich liegt es
...
Funktionswerte zu gewichten, denn die werden um so gr\"o\3er, je kleiner der Funktionswert
...
\end{equation}
wenn wir f\"ur $x$ den Wert $b$ und f\"ur $g(x)$ den Wert $f(b)$ einsetzen.  Subtrahieren wir
...
\item[I.S.:] $n \mapsto n+1$
      ...
      $
\end{enumerate}
Ähnlich wie beim Beweis des Zwischenwert-Satzes l\"asst sich zeigen, dass die Folge
...
immer gr\"o\3er als $a$, konvergieren beide Folgen.  Allerdings ist nicht garantiert, dass
...
\item[I.S.:] $n \mapsto n + 1$.
      ...
      Die Funktion $f := (x \mapsto x^4 - 1)$ ist f\"ur nichtnegative Zahlen monoton steigend,
      dass hei\3t aus $0 \leq u \leq v$ folgt auch $f(u) \leq f(v)$.  Es gilt 
      \\[0.2cm]
      \hspace*{1.3cm}
      $f(n \cdot 10^{-3}) = n^4 \cdot 10^{-12} - 1$ \quad und \quad $f(10) = 10^4 - 1$.
      \\[0.2cm]
      Nach Induktions-Voraussetzung k\"onnen wir $a_n$ durch $n\cdot 10^{-3}$ absch\"atzen und 
      aufgrund der Monotonie von $f$ k\"onnen wir dann $f(a_n)$ durch $f(n\cdot 10^{-3})$
      absch\"atzen.
      ...
      $c_n'$ gr\"o\3er sein als der wahre Wert von $c_n$, der in dem Algorithmus tats\"achlich auftritt.
      Es gilt:
      \\[0.2cm]
      \hspace*{1.3cm}
    $
