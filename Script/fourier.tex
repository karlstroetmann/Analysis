...
wird in der Praxis zur Ton- und Bild-Verarbeitung eingesetzt.  Außerdem ist die
Fourier-Analyse ein wichtiges Hilfsmittel zur Lösung von Differential-Gleichungen.
Im Rahmen dieser Vorlesung werden wir die Fourier-Analyse allerdings nur zur Berechnung
unendlichen Reihen einsetzen, denn für die anderen Anwendungen reicht die Zeit 
nicht aus.
...
Die zentrale Frage bei der \emph{Fourier-Analyse} ist es, für eine gegebene periodische Funktion
...
$n\in\mathbb{N}$ mit $n\geq 1$ ist.  Anschließend integrieren wir  über das
...
durch partielle Integration.  Für $n\not=0$ gilt
\\[0.1cm]
\hspace*{1.3cm}
$
\begin{array}[t]{lcl}
...
Wir wollen uns mit einem Satz begnügen, der im wesentlichen auf Dirichlet 
(Johann Peter Gustav Lejeune Dirichlet; 1805 - 1859) zurück geht.
...
\begin{Definition}[Einschränkung einer Funktion]
  ...
  Intervall, so definieren wir die \emph{Einschränkung} von $f$ auf $[a,b]$ als die
  Funktion 
  \\[0.1cm]
  \hspace*{1.3cm}
...
\begin{Definition}[stückweise stetig differenzierbar]
  Eine Funktion $f:[0,2\!\cdot\!\pi] \rightarrow \mathbb{R}$ ist \emph{stückweise stetig differenzierbar} 
...
\begin{Satz} Es gelte
  ...
  \item $a_k = \bruch{1}{\pi} \cdot\int\limits_0^{2\cdot\pi}f(x)\cdot\cos(k\!\cdot\!x)\,\dr x$ \quad und 
        \quad  $b_k = \bruch{1}{\pi}\cdot\int\limits_0^{2\cdot\pi}f(x)\cdot\sin(k\!\cdot\!x)\,\dr x$,  
  \end{enumerate}
  dann gilt \\[0.1cm]
  \hspace*{1.3cm}
  $f(x) = \bruch{1}{2}\cdot a_0 \;+\; \sum\limits_{k=1}^\infty a_k\cdot \cos(k\!\cdot\!x) \;+\;
...
\noindent
Der Beweis dieses Satzes benötigt Hilfsmittel, die im Rahmen der Vorlesung
nicht eingeführt werden können.

\section{Beispiele}
Um es gleich bei der Berechnung der Fourier-Koeffizienten einfacher zu haben, definieren
wir für eine Funktion $f:\mathbb{R} \rightarrow \mathbb{R}$ die Begriffe \emph{gerade}
und \emph{ungerade}: 
\begin{enumerate}
...
Wir berechnen als erstes die Fourier-Reihe für die \emph{Sägezahn}-Funktion $s:\mathbb{R} \rightarrow \mathbb{R}$,  
...
\item Die Koeffizienten $a_n$ ergeben sich als 
      ...
      $f(x) = \frac{1}{n}\cdot \sin(n\!\cdot\!x)$ und $g'(x) = 1$, also haben wir für 
      $n \geq 1$
      \\[0.3cm]
      \hspace*{1.3cm}
      $
...
\\[0.3cm]
gefunden.  Setzen wir hier für $x$ den Wert $\pi$ ein, so ergibt sich wegen $\cos\bigl((2\!\cdot\!k+1)\cdot \pi\bigr) = - 1$
\\[0.1cm]
\hspace*{1.3cm}
$
\begin{array}[t]{clcl}
...
Führende Mathematiker des 17-ten Jahrhunderts hatten sich erfolglos mit dieser Frage
beschäftigt.
Im Jahre 1735 gelang es Leonard Euler (1707 - 1783), dieses Problem zu lösen.
\pagebreak
...
\item Berechnen Sie mit Hilfe der Fourier-Reihe von $p$ einen Wert für die Reihe
      \\[0.3cm]
      \hspace*{1.3cm}
      $\displaystyle \sum\limits_{n=1}^\infty \bruch{1}{n^2}$. \eox
\end{enumerate}
