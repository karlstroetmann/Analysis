...
$
\\[0.3cm]
und $\lim\limits_{h \rightarrow 0} f(\widehat{x}+h) = f(\widehat{x})$ hei\3t gerade, dass $f$
...
\item Die Ableitung des Cosinus k\"onnte in analoger Weise berechnet werden, es ist aber 
...
\begin{Satz}[\href{http://en.wikipedia.org/wiki/Michel_Rolle}{Michel Rolle}, \label{satz:rolle} 1652 -- 1719]
  Ist die Funktion \mbox{$f:[a,b]\rightarrow \mathbb{R}$} differenzierbar und gilt au\3er\-dem $f(a) = f(b)$, dann gibt es ein
...
Der folgende Satz ist f\"ur die praktische Berechnung von Grenzwerten unentbehrlich.
...
\begin{Definition}[monoton]
  ...
  \\[0.2cm]
  erf\"ullt ist.  Weiter hei\3t  $f$ \emph{monoton fallend}, wenn
  \\[0.2cm]
  \hspace*{1.3cm}
...
\item[``$\Rightarrow$'':]  Wir nehmen zun\"achst an, dass $f$ monoton steigend ist und zeigen, dass
      ...
      \\[0.3cm]
      f\"ur alle $h \not = 0$ gr\"o\3er oder gleich 0 ist.  Zum Nachweis dieser Behauptung f\"uhren wir
      ...
      \item Fall: $h < 0$.
            ...
            \hspace*{1.3cm} $\bruch{f(x+h) - f(x)}{h} \geq 0$.
      \end{enumerate}
      Da der Differential-Quotient in jedem Fall gr\"o\3er-gleich $0$ ist und die Ableitung $f'(x)$ als
...
Gr\"o\3e von $f''(x)$ und $f''(x_0)$ durch.
\begin{enumerate}
...
von $x_0$ streng monoton steigend ist.  Da au\3erdem $f'(x_0) = 0$ gilt, folgt insgesamt
\\[0.2cm]
\hspace*{1.3cm}
...
Eine Funktion $f:\mathbb{R} \rightarrow \mathbb{R}$ hei\3t \emph{konvex} genau dann, wenn 
\\[0.2cm]
\hspace*{1.3cm}
...
Eine Funktion $f:\mathbb{R} \rightarrow \mathbb{R}$ hei\3t \emph{konkav} genau dann, wenn 
\\[0.2cm]
\hspace*{1.3cm}
...
\item[``$\Leftarrow$'':]  Es seien $x_1$, $x_2$ und $t \in [0,1]$ gegeben.  O.B.d.A. sei weiter
  ...
  \\[0.2cm]
  gilt.  Da $f''(x) \geq 0$ ist, wissen wir au\3erdem, dass die Funktion $f'(x)$ monoton steigend ist.
...
\\[0.2cm]
gilt.  Überlegen  Sie, wie Sie diese Aussage so verallgemeinern k\"onnen, dass die verallgemeinerte Aussage
