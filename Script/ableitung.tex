...
$f:D \rightarrow \mathbb{R}$ abgeschw\"acht wird, wenn also $f$ nicht mehr auf ganz $\mathbb{R}$
definiert ist.  Dann muss allerdings zus\"atzlich sichergestellt werden,  
dass es eine $\varepsilon$-Umgebung von $\bar{x}$ gibt, die vollst\"andig in dem Definitions-Bereich
$D$ liegt, es muss also ein $\varepsilon > 0$ geben, so dass $U_\varepsilon(\bar{x}) \subseteq D$ gilt.
\eox

\exercise
\"Uberlegen Sie, warum diese zus\"atzliche Bedingung notwendig ist. \eox
