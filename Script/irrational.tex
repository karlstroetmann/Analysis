\chapter{Die Kreiszahl $\pi$ und die Euler'sche Zahl $e$ sind irrational}
In diesem Kapitel zeigen wir, dass sowohl die Kreiszahl $\pi$, die als Fläche eines Kreises mit dem Radius 1
definiert ist, als auch die Euler'sche Zahl $e$, die als Grenzwert der Reihe
\\[0.2cm]
\hspace*{1.3cm}
$\exp(1) := \sum\limits_{k=0}^\infty \bruch{1}{k!}$
\\[0.2cm] 
festgelegt ist, irrational sind.  Da der Nachweis der Irrationalität von $e$ einfacher ist, beginnen wir
damit.
...
\begin{equation}
  ...
  0 < r_n,
\end{equation}
denn der $n$-te Rest $r_n$ enthält auf jeden Fall den Term $\frac{1}{(n+1)!}$ und der ist positiv.  
Wir wollen nun den $n$-ten Rest $r_n$ nach oben hin abschätzen.  Dazu benötigen wir zunächst die folgende
Abschätzung, die für alle $k > n + 1$ gültig ist:
\\[0.2cm]
\hspace*{1.3cm}
$
\begin{array}[t]{llcl}
...
\\[0.2cm]
\hspace*{1.3cm}
$(n+1) + i > n+1$ \quad für $i \in \{1, \cdots, k-(n+1)\}$,
\\[0.2cm]
...
\begin{equation}
 ...
 \bruch{(n+1)!}{k!} < \bruch{1}{(n+1)^{k-(n+1)}}  \quad \mbox{für alle $k > n+1$}
\end{equation}
und für $k = n + 1$ haben wir die Gleichheit beider Seiten.
Nun gilt für alle $n \in \mathbb{N}$ mit $n \geq 1$ die folgende Ungleichungs-Kette:
\\[0.2cm]
\hspace*{1.3cm}
$
\begin{array}[t]{lcll}
...
\proof
Wir nehmen an, dass $e$ rational ist.  Dann gibt es natürliche Zahlen $p,q \in \mathbb{N}$ mit
$q \geq 1$ und
\\[0.2cm]
\hspace*{1.3cm}
$e = \bruch{p}{q}$.
\\[0.2cm]
Wir betrachten den Ausdruck $q! \cdot r_q = q! \cdot (e - s_q)$ und setzten dort für $e$ den Wert $\frac{p}{q}$ ein:
\\[0.2cm]
\hspace*{1.3cm}
...
$,
\\[0.2cm]
denn $(q-1)! \cdot p$ ist auf jeden Fall eine natürliche Zahl und für $k \leq q$ hat der Ausdruck
$\frac{q!}{k!}$ die Form
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$0 < q! \cdot r_q < 1$  \quad und \quad $q! \cdot r_q \in \mathbb{Z}$ 
\\[0.2cm]
...
\\
\hspace*{1.3cm}
$S_n := \sum\limits_{k=0}^n (-1)^k \cdot \bigl(f^{(2k)}(\pi) + f^{(2k)}(0)\bigr) + 
  (-1)^{n+1} \cdot \dint{0}{\pi} f^{(2n+2)}(x) \cdot \sin(x)\, dx
$.
\\[0.2cm]
Die Behauptung
...
im Induktions-Schritt zwei partielle Integrationen durchführen.  
\begin{enumerate}
\item[I.A.:] $n=0$.
     ...
     $  - f(x) \cdot \cos(x) \Bigr|_0^\pi = - f(\pi) \cdot \cos(\pi) + f(0) \cdot \cos(0)
      = f(\pi) + f(0)$.
     \\[0.2cm]
     Um das verbleibende Integral zu berechnen führen wir eine erneute partielle Integration durch, bei
     der wir diesmal $u(x) = f'(x)$ und $v'(x) = \cos(x)$ setzen.  Dann gilt $u'(x) = f^{(2)}(x)$ und
     $v(x) = \sin(x)$.  Damit finden wir für das Integral $I$ den Wert
     \\[0.2cm]
     \hspace*{1.3cm}
     ...
     $
     \begin{array}[t]{lcl}
       S_0 & = & \sum\limits_{k=0}^0 (-1)^k \cdot \bigl(f^{(2k)}(\pi) + f^{(2k)}(0)\bigr) + 
...
\item[I.S.:] $n \mapsto n + 1$
     ...
     $v'(x) = \cos(x)$.  Dann haben wir $u'(x) = f^{(2n+3)}(x)$ und $v(x) = \sin(x)$ und für das Integral
     finden wir
     \\[0.2cm]
     \hspace*{1.3cm}
     $
     ...
     $
     \begin{array}[t]{lcl}
      I & \stackrel{IV}{=} & \sum\limits_{k=0}^n (-1)^k \cdot \bigl(f^{(2k)}(\pi) + f^{(2k)}(0)\bigr) + 
                 (-1)^{n+1} \cdot J_n 
             \\[0.4cm]
        & = & \sum\limits_{k=0}^n (-1)^k \cdot \bigl(f^{(2k)}(\pi) + f^{(2k)}(0)\bigr) 
             + (-1)^{n+1} \cdot \Bigl( f^{(2n+2)}(\pi) + f^{(2n+2)}(0) - J_{n+1} \Bigr)
            \\[0.4cm]
        & = & \sum\limits_{k=0}^{n+1} (-1)^k \cdot \bigl(f^{(2k)}(\pi) + f^{(2k)}(0)\bigr) 
             + (-1)^{n+2} \cdot J_{n+1} \; = \;S_{n+1} 
            \\[0.4cm]
     \end{array}
     $
     \\[0.2cm]
...
\proof
Wir führen den Beweis indirekt und nehmen an, dass $\pi \in \mathbb{Q}$ ist.  Dann gibt es Zahlen
$p,q \in \mathbb{N}$ mit 
\\[0.2cm]
\hspace*{1.3cm}
$\pi = \bruch{p}{q}$.
...
Setzen wir in der Behauptung des letzten Lemmas für $f$ die Funktion $g_n(x)$ ein, so folgt daher
\\[0.2cm]
\hspace*{1.3cm}
...
Dabei reicht es aus, dies für die Summanden der Form $g_n^{(2k)}(0)$ zu zeigen, denn es gilt
\\[0.2cm]
\hspace*{1.3cm}
$g_n^{(2k)}(\pi) = (-1)^{2\cdot k} \cdot g_n^{(2k)}(\pi - \pi) = g_n^{(2k)}(0)$.
\\[0.2cm]
Wir zeigen mit Hilfe einer Fallunterscheidung, dass    
$g_n^{(k)}(0) \in \mathbb{N}$ für alle $k \in \mathbb{N}$ gilt.
\begin{enumerate}
\item Fall: $k < n$.
      ...
      \\[0.2cm]
      gilt.  Jeder Term dieser Summe enthält mindestens den Faktor $x$.
      Setzen wir hier für $x$ den Wert $0$ ein, so wird daher jeder Term in der Summe $0$.  Damit
      gilt 
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item Fall: $k \geq n$.
      ...
      \\[0.2cm]
      denn wenn $k \geq n$ ist, ist $\bruch{k!}{n!}$ eine natürliche Zahl.
\end{enumerate}
...
Außerdem gilt für alle $x \in [0,\pi]$
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Daher gibt es ein $n \in \mathbb{N}$, so dass $\pi \cdot \bruch{\pi^n \cdot p^n}{n!} < 1$ ist und für dieses
$n$ haben wir 
\\[0.2cm]
\hspace*{1.3cm}
$0 < I_n < 1$.
...
Eine Zahl $r \in \mathbb{R}$ heißt \emph{algebraisch} genau dann, wenn es ein Polynom
\\[0.2cm]
\hspace*{1.3cm}
$p(x) = \sum\limits_{i=0}^n a_i \cdot x^i$  \quad mit $a_i \in \mathbb{Z}$ für alle $i = 0, 1, \cdots, n$
\\[0.2cm]
...
denn sonst könnten wir zu beliebigem $r \in \mathbb{R}$ einfach das Polyom
\\[0.2cm]
\hspace*{1.3cm}
$p_r(x) := x - r$
\\[0.2cm]
...
\exercise
Zeigen Sie, dass die Zahl $\sqrt{2\,} + \sqrt{3\,}$ eine algebraische Zahl ist.  Zeigen Sie außerdem,
dass diese Zahl keine rationale Zahl ist.  \eod

\begin{Definition}[Transzendente Zahl]
  Eine Zahl $x \in \mathbb{R}$ ist genau dann \emph{transzendent}, wenn $x$ nicht algebraisch ist.
\end{Definition}

Es läßt sich zeigen, dass die Menge aller Polynome mit ganzzahligen Koeffizienten abzählbar ist.  Damit
ist natürlich auch die Menge der algebraischen Zahlen abzählbar.  Da die Menge der reellen Zahlen
überabzählbar ist, muss es also sehr viele reelle Zahlen geben, die transzendent sind.  Allerdings ist
der Nachweis der Transzendenz einer Zahl in der Regel recht aufwändig.  
...
Der Nachweis dieser beiden Theoreme gehört zu
den wichtigsten mathematischen Erkenntnissen des 19ten Jahrhunderts.  Für diese Vorlesung sind diese
Beweise zu aufwändig, wir könnten diese Beweise aber mit den uns jetzt zur Verfügung stehenden Mitteln
führen.

%Unter
%\\[0.2cm]
%\hspace*{1.3cm}
%\texttt{http://www.mathematik.uni-muenchen.de/\symbol{126}fritsch/euler.pdf}
%\\[0.2cm]
...
%\\[0.2cm]
%\hspace*{1.3cm}
%\texttt{http://www.mathematik.uni-muenchen.de/\symbol{126}fritsch/pi.pdf}.

\exercise 
Zeigen Sie, dass für alle natürlichen Zahlen $n \in \mathbb{N}$
\\[0.2cm]
\hspace*{1.3cm}
...
\noindent
\textbf{Bemerkung}:  Die letzte Gleichung motiviert die folgende Definition der
...
$\Gamma(n+1) = n!$
\\[0.2cm]
und daher können wir die Gamma-Funktion als eine Erweiterung der Fakultäts-Funktion auf
die natürlichen Zahlen {auf}fassen.
