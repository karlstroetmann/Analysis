...
trigonometrischen Funktionen effektiv mit Hilfe von Reihen berechnen lassen.   Anschlie\3end diskutieren
...
wenn die Funktion, deren Nullstelle bestimmt werden soll, differenzierbar ist.  Au\3erdem untersuchen 
...
(\ref{eq:taylor}) $k$-mal nach $x$ differenzieren und anschlie\3end $x = 0$ setzen.
...
\\[0.3cm]
Der Abbruch-Fehler gibt also an, wie gro\3 der Fehler ist, wenn wir die Berechnung der Summe einer
...
Absch\"atzung f\"ur den Abbruch-Fehler an.  Folgern Sie au\3erdem die
...
\\[0.3cm] 
Die Ableitung der Taylor-Reihe muss diese Reihe ergeben und au\3erdem muss die Reihe an der
...
Formel hei\3t das, dass der Abbruch-Fehler wie folgt abgesch\"atzt werden kann:
\\[0.3cm]
\hspace*{1.3cm}
$\left|\arctan(x) - \sum\limits_{k=0}^n (-1)^k \cdot  \bruch{1}{2\cdot k+1}\right| \leq \bruch{1}{2\cdot (n+1)+1}$
\\[0.3cm]
Überlegen wir, wieviele Glieder der Summe ben\"otigt werden, um $\ds\frac{\pi}{4}$ auf eine Genauigkeit von $10^{-9}$ 
...
Differenzen der Ordnung $k$ durch Bildung einer Differenz und einer anschlie\3enden
...
Wir untersuchen als n\"achstes, wie gro\3 der Fehler bei der Polynom-Interpolation werden
...
\item[I.S.:] $k\mapsto k+1$.  Nach Induktions-Voraussetzung hat die Funktion
             ...
             Nehmen wir an, diese Nullstellen seien der Gr\"o\3e nach geordnet als
             \\[0.2cm]
...
\\[0.2cm]
Mit $f$ ist auch die Funktion $g$ mindestens $(n+1)$-mal differenzierbar.  Au\3erdem gilt
...
\\[0.2cm]
denn der Faktor $x_k-x_i$ verschwindet im Falle $i=k$. Au\3erdem haben wir 
\\[0.2cm]
\hspace*{1.3cm}
$
\begin{array}[t]{lcl}
...
\\[0.2cm]
erf\"ullt ist.  Das ist der Grund f\"ur die \"au\3erst langsame Konvergenz der Folge $\folge{x_n}$.
\eox 
...
konvergiert das Verfahren gegen die L\"osung $-2.23070764576\cdots$, die au\3erhalb des Intervalls $[-2,2]$
...
konvergiert und wollen au\3erdem die Geschwindigkeit der Konvergenz des Verfahrens untersuchen.
...
$f^{(2)}(\varphi) \geq 0$ 
\\[0.2cm]
und das Quadrat $(x-x_0)^{2}$ ist sicher immer gr\"o\3er oder gleich $0$.  \qed
...
$f'(x)$ f\"ur alle $x \in [a,b]$ echt gr\"o\3er als Null ist, kann $f$ keine zweite Nullstelle haben, denn
...
\\[0.2cm]
Wir subtrahieren $f(x_n)$ und teilen anschlie\3end durch $f'(x_n)$.  Das liefert die Gleichung
\\[0.2cm]
\hspace*{1.3cm}
$\ds - \frac{f(x_n)}{f'(x_n)} = 
...
\\[0.2cm]
zu l\"osen, haben Sie im ersten Semester kennegelernt: Es ist das Gau\3'sche
Eliminations-Verfahren (Carl Friedrich Gau\3, 1777 -- 1855).  
Es gibt allerdings Situationen, in denen dieses
Verfahren zu 
aufwendig ist.  Dies ist beispielsweise dann der Fall, wenn einerseits $n$ gro\3 ist und
wenn andererseits die meisten Komponenten der Matrix $A$ den Wert 0 haben.  Solche
Matrizen hei\3en \emph{d\"unn besetzte} Matrizen.  Diese Art von Matrizen tritt bei
...
Das Gau\3'sche Eliminations-Verfahren besitzt  eine
Komplexit\"at von $O(n^3)$ und ist damit f\"ur gro\3e d\"unn besetzte Matrizen ungeeignet.
Hier ist der Rechenaufwand bei iterative Verfahren geringer.   
Ein weiteres Problem ist, dass die Rundungsfehler beim Gau\3'schen Eliminations-Verfahren
sehr gro\3 werden k\"onnen.  Demgegen\"uber sind iterative Verfahren selbstkorrigierend.
...
$\norm{\vec{x} + \vec{y}} \leq \norm{\vec{x}} + \norm{\vec{y}}$.
\\[0.3cm]
Au\3erdem haben wir f\"ur reelle Zahlen $\alpha \in\mathbb{R}$ und Vektoren
...
Schlie\3lich gilt f\"ur alle $\vec{x}\in\mathbb{R}^n$ die Ungleichung 
...
\begin{Satz}
  ...
  \\[0.2cm] 
  und ist weiter $q$ die Zahl aus dem starken Zeilen-Summen-Kriterium, dann gilt au\3erdem die
  Absch\"atzung 
  \\[0.2cm]
  \hspace*{1.3cm}
...
\end{enumerate}
Die Funktion \texttt{demo()} ruft die Funktion so auf, dass anschlie\3end
...
\subsubsection{Das Gau\3-Seidel-Verfahren}
...
auch \emph{Gau\3-Seidel-Verfahren} (Philipp Ludwig von Seidel, 1821 -- 1896) bezeichnet.

Abbildung \ref{fig:seidel.stlx} zeigt, wie wir die Implementierung \"andern
m\"ussen, wenn wir das Gau\3-Seidel-Verfahren anwenden wollen.  Die Implementierung des
Gau\3-Seidel-Verfahrens ist einfacher als die Implementierung des Jacobi-Verfahrens, denn
wir ben\"otigen keine Hilfsvariable \texttt{xNew} mehr.
Tabelle \ref{tab:seidel} zeigt, dass das Gau\3-Seidel-Verfahren f\"ur
unser Beispiel etwa doppelt so schnell konvergiert wie das Jacobi-Verfahren.
Es l\"asst sich zeigen, dass das Gau\3-Seidel-Verfahren zur L\"osung immer dann konvergiert, wenn die Matrix
...
Gau\3-Seidel-Verfahren schwieriger als die Untersuchung des Jacobi-Verfahrens, so dass wir von einer
...
\begin{Verbatim}[ frame         = lines, 
    ...
    \end{Verbatim}
\vspace*{-0.3cm}
  \caption{Implementierung des Gau\3-Seidel-Verfahrens.}
...
\hline
  \end{tabular}}
  \caption{Konvergenz des Gau\3-Seidel-Verfahrens.}
  \label{tab:seidel}
\end{table}
