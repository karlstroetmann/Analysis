...
\item Fall: $j\not=k$.  Dann haben wir 
      ...
      & = & \ds\frac{(x_j - x_0) \cdot  \cdots \cdot  (x_j - x_j) \cdot  \cdots \cdot (x_j - x_n)}{(x_k - x_0) \cdot  \cdots \cdot  (x_k - x_j) \cdot  \cdots \cdot (x_k - x_n)} \\[0.3cm]
      & = & 0 \\[0.1cm]
      & = & \delta_{j,k}
      \end{array}
      $
...
\begin{array}[t]{lcl}
  g(\bar{x}) & = & \ds f(\bar{x}) - p(\bar{x}) - \left(\prod\limits_{i=0}^{n} \bruch{\bar{x}-x_i}{\bar{x}-x_i}\right) \cdot  \bigl(f(\bar{x}) - p(\bar{x})\bigr) \\[0.5cm]
             & = & \ds f(\bar{x}) - p(\bar{x}) - 1\cdot \bigl(f(\bar{x}) - p(\bar{x})\bigr) \\[0.1cm]
             & = & 0.
\end{array}
$
\\[0.1cm]
...
\\[0.3cm]
\hspace*{0.8cm}
$\ds|x_{i+2} - x_{i+1}| = |f(x_{i+1}) - f(x_i)| \leq q \cdot  |x_{i+1} - x_i| \stackrel{IV}{\leq}
...
$
\begin{array}[t]{lcl}
                & |x_n - \bar{x}| \leq \varepsilon \\[0.2cm]
...
\Leftrightarrow & (1 + \alpha) \cdot  x & = & f(x) + \alpha \cdot  x                    & \mid\; \cdot  \;\bruch{1}{1 + \alpha} \\[0.2cm]
...
\begin{equation}
  \label{eq:NewtonZero}
  \colorbox{orange}{\framebox{$\ds x_{n+1} = x_n - \bruch{g(x_n)}{g'(x_n)}$}} 
\end{equation}
...
$.
\\[0.2cm]
An dieser Stelle bemerken wir, dass $x_n - \bruch{f(x_n)}{f'(x_n)} = x_{n+1}$ ist und haben damit
\\[0.2cm]
\hspace*{1.3cm}
$\ds x_{n+1} - \bar{x} = \frac{1}{2} \cdot \bruch{f^{(2)}(\varphi)}{f'(x_n)} \cdot (\bar{x} - x_n)^2$.
\\[0.2cm]
