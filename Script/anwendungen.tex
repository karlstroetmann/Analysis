...
\\[0.2cm]
mit $\chi \in[0,x]$ und für $x \in (-1, 1]$ geht dieser Wert für $n \rightarrow \infty$ gegen $0$.
Damit haben wir insgesamt $\textsl{taylor}\bigl(x \mapsto \ln(1+x), x \bigr) = \ln(1+x)$ für $x \in (-1, 1]$ gezeigt und folglich können
wir
\\[0.2cm]
\hspace*{1.3cm}
...
Abschätzung für den Abbruch-Fehler an.  Folgern Sie außerdem die auf 
\href{http://en.wikipedia.org/wiki/Leonard_Euler}{Leonard Euler} (1707 --- 1783) zurück gehende 
...
\\[0.2cm]
bei der $i$ die imaginäre Einheit bezeichnet, es gilt also $i\cdot i = -1$. \eox
...
\\[0.3cm]
Überlegen wir, wieviele Glieder der Summe benötigt werden, um $\ds\frac{\pi}{4}$ auf eine Genauigkeit von $10^{-9}$ 
...
\\[0.2cm]
definieren wir für alle $k\in\{0,1,\cdots,n\}$ das $k$-te \emph{Lagrange'sche Polynom} 
...
\end{equation}
Diese Eigenschaft lässt sich durch einfaches Nachrechnen bestätigen.  Wir betrachten die Fälle
$j=k$ und $j\not=k$ getrennt:
\begin{enumerate}
...
$\pair(x_0,y_0)$, $\pair(x_1,y_1)$, $\cdots$, $\pair(x_n,y_n)$,
\\[0.2cm]
genau ein Polynom $p(x)$ gibt, so dass $p(x_i) = y_i$ für alle $i=0,1,\cdots,n$ gilt. \eox
...
reellen Zahlen wird dann durch einen \emph{vollständigen}
...
Fixpunkt-Verfahren motivieren.  Wir haben bereits einmal ein solches angewendet:  In dem Kapitel
...
$x = f(x)$ 
\\[0.2cm]
iterativ bestimmen wollen, indem wir induktiv eine Folge $\folge{x_n}$ definieren, wobei 
$x_{n+1} = f(x_n)$ gelten soll.  Den Startwert $x_1$ werden wir dabei weitgehend willkürlich
wählen.  Zunächst ist Folgendes klar:  Sollte die Folge $\folge{x_n}$ gegen einen Grenzwert
$\bar{x}$ konvergieren, so ist dieser Grenzwert eine Lösung der Fixpunkt-Gleichung $x = f(x)$, denn
es gilt
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
die Stetigkeit von $f$ ausgenutzt.  Weniger klar ist die Antwort auf die Frage, wann die durch
$x_{n+1} = f(x_n)$ definierte Folge konvergiert.  Die entscheidende Antwort auf diese Frage hat
Stefan Banach gegeben:  Wenn die Funktionswerte von $f$ näher beieinander liegen als die Argumente,
...
$\bar{x}$ hat und nehmen wir weiter an, dass eine (nicht besonders gute) Näherung $x_1$ für
...
$[0,1]$ monoton fallend ist, denn $\dfo \cos(x) = -\sin(x)$  und für alle $x \in \bigl[0,\pi\bigr]$
gilt $\sin(x) \geq 0$.
...
\noindent
\textbf{Hinweis}:  Diese Aufgabe ist in erster Linie als Programmier-Aufgabe gedacht.
\eox
...
\emph{schwach kontrahierende Abbildung} bezeichnen.  In der Tat kann gezeigt werden, dass auch für
...
$x_{n+1} = \cos(x_n)$ ist, müssen wir damit rechnen, dass die Rundungfehler höher sind als bei der
...
Weiter sehen Sie, dass durch die Nach-Iteration die letzte angezeigte Stelle verändert wird.
